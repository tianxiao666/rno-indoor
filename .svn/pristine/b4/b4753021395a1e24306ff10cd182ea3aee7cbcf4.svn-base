var env = env || {};
if (!!!env.svgeditoffset) {
    env.svgeditoffset = 0;
}
var viewboxPosFix = true;
var viewboxPosX = 16;
var viewboxPosY = 16;
var embed = false;
function setEmbed() {
    embed = true;
    $("#change_resolution_div").attr("style", "display: none");
    $("#tool_export").attr("style", "display: none");
}

jQuery && function () {
    var a = $(window), K = $(document);
    $.extend($.fn, {
        contextMenu: function (l, s) {
            ...
            $(this).each(function () {
                var v = $(this), G = $(v).offset(), e = $("#" + l.menu);
                ...
                $(this).bind("mousedown", function (f) {
                    $(this).mouseup(function (k) {
                        ...
                        if (f.button === 2 || l.allowLeft || f.ctrlKey && svgedit.browser.isMac()) {
                            ...
                            var F = k.pageX, B = k.pageY;
                            k = a.width() - e.width();
                            var A = a.height() - e.height();
                            if (F > k - 15) F = k - 15;
                            if (B > A - 30) B = A - 30;
                            K.unbind("click");
                            e.css({top: B, left: F}).fadeIn(l.inSpeed);
                            ...
                            K.keypress(function (O) {
                                switch (O.keyCode) {
                                    case 38:
                                        ...
                                        break;
                                    case 40:
                                        if (e.find("LI.hover").length == 0) e.find("LI:first").addClass("hover"); else {
            ...
            $(this).each(function () {
                if (l != undefined)for (var s = l.split(","),
                                            v = 0; v < s.length; v++)$(this).find('A[href="' + s[v] + '"]').parent().addClass("disabled")
            });
            return $(this)
            ...
            $(this).each(function () {
                if (l !=
                    undefined)for (var s = l.split(","),
                                       v = 0; v < s.length; v++)$(this).find('A[href="' + s[v] + '"]').parent().removeClass("disabled")
            });
            return $(this)
...
(function () {
    ...
    if (svgedit.browser.supportsSvg()) {
        var K = navigator.userAgent, l = document.createElementNS("http://www.w3.org/2000/svg", "svg"),
            s = !!window.opera, v = K.indexOf("AppleWebKit") >= 0, G = K.indexOf("Gecko/") >= 0,
            e = K.indexOf("MSIE") >= 0, f = K.indexOf("Chrome/") >= 0, k = K.indexOf("Windows") >= 0,
            n = K.indexOf("Macintosh") >=
                0, F = "ontouchstart" in window, B = !!l.querySelector, A = !!document.evaluate, O = function () {
...
(function () {
    ...
    var a = document.createElementNS("http://www.w3.org/2000/svg", "svg"), K = {};
    svgedit.transformlist.SVGTransformList = function (l) {
        ...
        this._init = function () {
            ...
            if (s)for (var v = /\s*((scale|matrix|rotate|translate)\s*\(.*?\))\s*,?\s*/, G = true; G;) {
                ...
                s = s.replace(v, "");
                if (G && G[1]) {
                    var e = G[1].split(/\s*\(/), f = e[0];
                    e = e[1].match(/\s*(.*?)\s*\)/);
                    e[1] = e[1].replace(/(\d)-/g, "$1 -");
                    var k = e[1].split(/[, ]+/), n = "abcdef".split(""), F = a.createSVGMatrix();
                    $.each(k, function (O, Z) {
                        k[O] = parseFloat(Z);
                        if (f == "matrix") F[n[O]] = k[O]
                    });
                    e = a.createSVGTransform();
                    var B = "set" + f.charAt(0).toUpperCase() + f.slice(1), A = f == "matrix" ? [F] : k;
                    if (f == "scale" && A.length == 1) A.push(A[0]); else if (f == "translate" && A.length == 1) A.push(0); else if (f == "rotate" && A.length == 1) {
                        A.push(0);
                        A.push(0)
                    }
                    e[B].apply(e, A);
    ...
    svgedit.transformlist.getTransformList = function (l) {
        ...
        else if (l.gradientTransform)return l.gradientTransform.baseVal; else {
...
(function () {
    ...
    svgedit.math.transformPoint = function (K, l, s) {
        return {x: s.a * K + s.c * l + s.e, y: s.b * K + s.d * l + s.f}
    };
    ...
    svgedit.math.transformBox = function (K, l, s, v, G) {
        var e = {x: K, y: l}, f = {x: K + s, y: l};
        s = {x: K + s, y: l + v};
        K = {x: K, y: l + v};
        l = svgedit.math.transformPoint;
        e = l(e.x, e.y, G);
        var k = v = e.x, n = e.y, F = e.y;
        f = l(f.x, f.y, G);
        v = Math.min(v, f.x);
        k = Math.max(k, f.x);
        n = Math.min(n, f.y);
        ...
        n = Math.min(n, s.y);
        F = Math.max(F, s.y);
        return {tl: e, tr: f, bl: K, br: s, aabox: {x: v, y: n, width: k - v, height: F - n}}
    };
    ...
    svgedit.math.snapToAngle = function (K, l, s, v) {
        ...
        return {x: K + v * Math.cos(G), y: l + v * Math.sin(G), a: G}
    };
...
(function () {
    ...
    var a = ["x", "x1", "cx", "rx", "width"], K = ["y", "y1", "cy", "ry", "height"], l = $.merge(["r", "radius"], a);
    $.merge(l, K);
    var s, v = {px: 1};
    ...
    svgedit.units.convertAttrs = function (e) {
        var f = e.tagName, k = s.getBaseUnit();
        if (f = G[f])for (var n = f.length, F = 0; F < n; F++) {
            var B = f[F], A = e.getAttribute(B);
    ...
    svgedit.units.convertToNum =
        function (e, f) {
            ...
            if (f.substr(-1) === "%") {
                var k = f.substr(0, f.length - 1) / 100, n = s.getWidth(), F = s.getHeight();
...
(function () {
    ...
    svgedit.utilities.encode64 = function (e) {
        ...
        var f = Array(Math.floor((e.length + 2) / 3) * 4), k, n, F, B, A, O, Z = 0, N = 0;
        do {
            k = e.charCodeAt(Z++);
            n = e.charCodeAt(Z++);
            F = e.charCodeAt(Z++);
            ...
            A = (n & 15) << 2 | F >> 6;
            O = F & 63;
            if (isNaN(n)) A = O = 64; else if (isNaN(F)) O = 64;
    ...
    svgedit.utilities.convertToXMLReferences = function (e) {
        for (var f = "", k = 0; k < e.length; k++) {
            ...
            if (n < 128) f += e[k]; else if (n > 127) f += "&#" + n + ";"
        }
        return f
    };
    ...
    svgedit.utilities.getUrlFromAttr = function (e) {
        if (e)if (e.indexOf('url("') === 0)return e.substring(5, e.indexOf('"', 6)); else if (e.indexOf("url('") === 0)return e.substring(5, e.indexOf("'", 6)); else if (e.indexOf("url(") === 0)return e.substring(4, e.indexOf(")"));
        return null
    };
    ...
    svgedit.utilities.getPathBBox = function (e) {
        var f = e.pathSegList, k = f.numberOfItems;
        e = [[], []];
        var n = f.getItem(0),
            F = [n.x, n.y];
        for (n = 0; n < k; n++) {
            ...
            if (typeof B.x != "undefined") {
                ...
                if (B.x1) {
                    for (var A = [B.x1, B.y1], O = [B.x2, B.y2], Z = [B.x, B.y], N = 0; N < 2; N++) {
                        ...
                        var L = 6 * F[N] - 12 * A[N] + 6 * O[N], na = -3 * F[N] + 9 * A[N] - 9 * O[N] + 3 * Z[N],
                            ca = 3 * A[N] - 3 * F[N];
                        if (na == 0) {
                            if (L != 0) {
                                L = -ca / L;
        ...
        return {x: f, y: n, width: k, height: e}
    };
    svgedit.utilities.getBBox = function (e) {
        ...
        switch (k) {
            ...
            case "path":
                ...
                } else e = svgedit.utilities.getPathBBox(f);
                break;
            case "g":
            case "a":
                e = a(f);
    ...
    svgedit.utilities.assignAttributes = function (e, f, k, n) {
        ...
        for (var F in f)if (k = F.substr(0, 4) === "xml:" ? "http://www.w3.org/XML/1998/namespace" : F.substr(0, 6) === "xlink:" ? "http://www.w3.org/1999/xlink" : null) e.setAttributeNS(k, F, f[F]); else n ? svgedit.units.setUnitAttr(e, F, f[F]) : e.setAttribute(F, f[F]);
        svgedit.browser.isOpera() || G.unsuspendRedraw(null)
    };
    svgedit.utilities.cleanupElement = function (e) {
        var f = G.suspendRedraw(60), k = {
            "fill-opacity": 1,
            "stop-opacity": 1,
            opacity: 1,
            stroke: "none",
    ...
    svgedit.sanitize.sanitizeSvg = function (v) {
        ...
        if (v.nodeType == 1) {
            ...
            if (v.ownerDocument && G) {
                var e = l[v.nodeName], f = s[v.nodeName];
                if (e != undefined) {
                    for (var k = [], n = v.attributes.length; n--;) {
                        var F = v.attributes.item(n), B = F.nodeName,
                            A = F.localName, O = F.namespaceURI;
                    ...
                    if (v.nodeName == "use" && !svgedit.utilities.getHref(v)) G.removeChild(v); else {
...
(function () {
    ...
    svgedit.history.ChangeElementCommand.prototype.apply = function (a) {
        ...
        for (l in this.newValues) {
            if (this.newValues[l])if (l == "#text") this.elem.textContent = this.newValues[l]; else l == "#href" ? svgedit.utilities.setHref(this.elem, this.newValues[l]) : this.elem.setAttribute(l, this.newValues[l]); else if (l == "#text") this.elem.textContent = ""; else {
    ...
    svgedit.history.ChangeElementCommand.prototype.unapply = function (a) {
        ...
        for (l in this.oldValues) {
            if (this.oldValues[l])if (l ==
                "#text") this.elem.textContent = this.oldValues[l]; else l == "#href" ? svgedit.utilities.setHref(this.elem, this.oldValues[l]) : this.elem.setAttribute(l, this.oldValues[l]); else if (l == "#text") this.elem.textContent = ""; else this.elem.removeAttribute(l);
    ...
    svgedit.history.BatchCommand.prototype.elements = function () {
        for (var a = [], K = this.stack.length; K--;)for (var l = this.stack[K].elements(),
                                                              s = l.length; s--;)a.indexOf(l[s]) == -1 && a.push(l[s]);
        return a
    };
    ...
    svgedit.history.UndoManager.prototype.beginUndoableChange = function (a, K) {
        ...
        this.undoableChangeStack[l] =
            {attrName: a, oldValues: v, elements: G}
    };
    svgedit.history.UndoManager.prototype.finishUndoableChange = function () {
        for (var a = this.undoChangeStackPointer--, K = this.undoableChangeStack[a], l = K.elements.length,
                 s = K.attrName, v = new svgedit.history.BatchCommand("Change " + s); l--;) {
            var G = K.elements[l];
            if (G != null) {
                var e = {};
                e[s] = K.oldValues[l];
...
(function () {
    ...
    svgedit.select.Selector.prototype.resize = function () {
        var v = this.selectorRect, G = l, e = G.selectorGrips, f = this.selectedElement,
            k = f.getAttribute("stroke-width"), n = a.currentZoom(), F = 1 / n;
    ...
    svgedit.select.SelectorManager.prototype.initGroup =
        function () {
            ...
            this.rotateGripConnector = this.selectorGripsGroup.appendChild(a.createSVGElement({
                element: "line",
                attr: {id: "selectorGrip_rotateconnector", stroke: "#22C", "stroke-width": "1"}
            }));
            ...
            if (!$("#canvasBackground").length) {
                ...
                G = a.createSVGElement({
                    element: "rect", attr: {
                        width: "100%", height: "100%", x: 0, y: 0, "stroke-width": 1,
    ...
    svgedit.select.SelectorManager.prototype.releaseSelector = function (v) {
        if (v != null)for (var G = this.selectors.length, e = this.selectorMap[v.id],
                               f = 0; f < G; ++f)if (this.selectors[f] && this.selectors[f] == e) {
...
(function () {
    ...
    var a = "a,circle,ellipse,foreignObject,g,image,line,path,polygon,polyline,rect,svg,text,tspan,use".split(","),
        K = {LET_DOCUMENT_DECIDE: 0, ALWAYS_RANDOMIZE: 1, NEVER_RANDOMIZE: 2}, l = K.LET_DOCUMENT_DECIDE;
    ...
    svgedit.draw.randomizeIds = function (s, v) {
        l = s == false ? K.NEVER_RANDOMIZE :
            K.ALWAYS_RANDOMIZE;
        if (l == K.ALWAYS_RANDOMIZE && !v.getNonce()) v.setNonce(Math.floor(Math.random() * 100001)); else l == K.NEVER_RANDOMIZE && v.getNonce() && v.clearNonce()
    };
    svgedit.draw.Drawing = function (s, v) {
        ...
        if (G && l != K.NEVER_RANDOMIZE) this.nonce_ = G; else l == K.ALWAYS_RANDOMIZE && this.setNonce(Math.floor(Math.random() * 100001))
    };
    ...
    svgedit.draw.Drawing.prototype.createLayer = function (s) {
        var v = this.svgElem_.ownerDocument, G = v.createElementNS("http://www.w3.org/2000/svg", "g");
...
(function () {
    ...
    svgedit.path.insertItemBefore = function (f, k, n) {
        f = f.pathSegList;
        if (svgedit.browser.supportsPathInsertItemBefore()) f.insertItemBefore(k, n); else {
    ...
    svgedit.path.getGripPt = function (f, k) {
        var n = {x: k ? k.x : f.item.x, y: k ? k.y : f.item.y}, F = f.path;
    ...
    svgedit.path.addPointGrip = function (f, k, n) {
        var F = svgedit.path.getGripContainer(), B = svgedit.utilities.getElem("pathpointgrip_" + f);
        if (!B) {
        ...
        k &&
        n && svgedit.utilities.assignAttributes(B, {cx: k, cy: n, display: "inline"});
        return B
    };
    ...
    svgedit.path.getPointGrip = function (f, k) {
        ...
        if (k) {
            ...
            svgedit.utilities.assignAttributes(n, {cx: F.x, cy: F.y, display: "inline"})
        }
        return n
    };
    svgedit.path.getControlPoints = function (f) {
        var k = f.item, n = f.index;
        if (!("x1" in k) || !("x2" in k))return null;
        var F = {};
        svgedit.path.getGripContainer();
        for (var B = [svgedit.path.path.segs[n - 1].item, k], A = 1; A < 3; A++) {
            var O = n +
                    "c" + A, Z = F["c" + A + "_line"] = svgedit.path.getCtrlLine(O),
                N = svgedit.path.getGripPt(f, {x: k["x" + A], y: k["y" + A]}),
                L = svgedit.path.getGripPt(f, {x: B[A - 1].x, y: B[A - 1].y});
            svgedit.utilities.assignAttributes(Z, {x1: N.x, y1: N.y, x2: L.x, y2: L.y, display: "inline"});
            F["c" + A + "_line"] = Z;
            pointGrip = F["c" + A] = svgedit.path.addCtrlGrip(O);
            svgedit.utilities.assignAttributes(pointGrip, {cx: N.x, cy: N.y, display: "inline"});
            F["c" + A] = pointGrip
        }
        return F
    };
    svgedit.path.replacePathSeg = function (f, k, n, F) {
        ...
        l[f]].apply(F, n);
        if (svgedit.browser.supportsPathReplaceItem()) F.pathSegList.replaceItem(f, k); else {
            n = F.pathSegList;
            F = n.numberOfItems;
            for (var B = [], A = 0; A < F; A++) {
                var O = n.getItem(A);
    ...
    svgedit.path.getSegSelector = function (f, k) {
        var n = f.index, F = svgedit.utilities.getElem("segline_" + n);
        if (!F) {
        ...
        if (k) {
            ...
            for (var A = 0; A < B.length; A += 2) {
                n = svgedit.path.getGripPt(f, {x: B[A], y: B[A + 1]});
                B[A] = n.x;
                B[A + 1] = n.y
            }
    ...
    svgedit.path.smoothControlPoints = this.smoothControlPoints =
        function (f, k, n) {
            var F = f.x - n.x, B = f.y - n.y, A = k.x - n.x, O = k.y - n.y;
            if ((F != 0 || B != 0) && (A != 0 || O != 0)) {
                f = Math.atan2(B, F);
                k = Math.atan2(O, A);
                F = Math.sqrt(F * F + B * B);
                ...
                if (f < 0) f += 2 * Math.PI;
                if (k < 0) k += 2 * Math.PI;
                var Z = Math.abs(f - k), N = Math.abs(Math.PI - Z) / 2;
                if (f - k > 0) {
                    f = Z < Math.PI ? f + N : f - N;
                    k = Z < Math.PI ? k - N : k + N
                } else {
    ...
    svgedit.path.Segment.prototype.update = function (f) {
        if (this.ptgrip) {
            ...
            svgedit.utilities.assignAttributes(this.ptgrip, {cx: k.x, cy: k.y});
    ...
    svgedit.path.Path.prototype.init = function () {
        ...
        var f = this.elem.pathSegList, k = f.numberOfItems;
        this.segs = [];
        this.selected_pts = [];
        this.first_seg = null;
        for (var n = 0; n < k; n++) {
        ...
        F = null;
        for (n = 0; n < k; n++) {
            var B = f[n], A = n + 1 >= k ? null : f[n + 1], O = n - 1 < 0 ? null : f[n - 1];
            if (B.type === 2) {
                if (O && O.type !== 1) {
                    A = f[F];
                    A.next = f[F + 1];
    ...
    svgedit.path.Path.prototype.addSeg = function (f) {
        ...
        if (k.prev) {
            ...
            switch (k.item.pathSegType) {
                case 4:
                    var B = (k.item.x + n.item.x) / 2, A = (k.item.y + n.item.y) / 2;
                    F = this.elem.createSVGPathSegLinetoAbs(B, A);
                    break;
                case 6:
                    F = (n.item.x + k.item.x1) / 2;
                    var O = (k.item.x1 + k.item.x2) / 2, Z = (k.item.x2 + k.item.x) / 2, N = (F + O) / 2;
                    O = (O + Z) / 2;
                    B = (N + O) / 2;
                    var L = (n.item.y + k.item.y1) / 2, na = (k.item.y1 + k.item.y2) / 2;
    ...
    svgedit.path.Path.prototype.deleteSeg = function (f) {
        var k = this.segs[f], n = this.elem.pathSegList;
        k.show(false);
        var F = k.next;
        if (k.mate) {
    ...
    svgedit.path.Path.prototype.setSegType = function (f) {
        ...
        for (var k = this.selected_pts.length, n; k--;) {
            var F = this.segs[this.selected_pts[k]], B = F.prev;
            if (B) {
                if (!f) {
                ...
                f -= 0;
                var A = F.item.x, O = F.item.y, Z = B.item.x;
                B = B.item.y;
                var N;
                switch (f) {
                    case 6:
    ...
    var e = function (f, k) {
        ...
        r = Math.sqrt(dx * dx + dy * dy);
        theta = Math.atan2(dy, dx) - angle;
        return {x: (r * Math.cos(theta) + newcx) / 1, y: (r * Math.sin(theta) + newcy) / 1}
    };
    svgedit.path.recalcRotatedPath = function () {
        var f = svgedit.path.path.elem, k = svgedit.utilities.getRotationAngle(f, true);
        if (k) {
            var n = svgedit.utilities.getBBox(f), F = svgedit.path.path.oldbbox, B = F.x + F.width / 2,
                A = F.y + F.height / 2;
            F = n.x + n.width / 2;
            n = n.y + n.height / 2;
            F = F - B;
...
(function () {
    ...
    jQuery.fn.attr = function (K, l) {
        ...
        for (var v = 0; v < s; v++) {
            var G = this[v];
            if (G.namespaceURI === "http://www.w3.org/2000/svg")if (l !== undefined) G.setAttribute(K, l); else if ($.isArray(K)) {
                s = K.length;
                for (v = {}; s--;) {
                    var e = K[s], f = G.getAttribute(e);
                    if (f || f === "0") f = isNaN(f) ? f : f - 0;
                    v[e] = f
                }
                return v
            } else if (typeof K === "object")for (e in K)G.setAttribute(e, K[e]); else {
...
$.SvgCanvas = function (a, K) {
    function l(b, c) {
        for (var d = svgedit.utilities.getBBox(b), m = 0; m < 2; m++) {
            var i = m === 0 ? "fill" : "stroke", z = b.getAttribute(i);
            if (z && z.indexOf("url(") === 0) {
                z = rb(z);
                if (z.tagName === "linearGradient") {
                    var o = z.getAttribute("x1") || 0, h = z.getAttribute("y1") || 0, q = z.getAttribute("x2") || 1,
    ...
    var s = "http://www.w3.org/2000/svg", v = {show_outside_canvas: true, selectNew: true, dimensions: [640, 480]};
    K && $.extend(v, K);
    var G = v.dimensions, e = this, f = a.ownerDocument,
    ...
    $.extend(Z.text, {
        fill: "#000000",
        stroke_width: 0, font_size: 24, font_family: "serif"
    });
    var N = Z.shape, L = Array(1), na = this.addSvgElementFromJson = function (b) {
            var c = svgedit.utilities.getElem(b.attr.id), d = B().getCurrentLayer();
            ...
            return c
        }, ca = e.getTransformList = svgedit.transformlist.getTransformList, oa = svgedit.math.transformPoint,
        pa = e.matrixMultiply = svgedit.math.matrixMultiply,
        ia = e.hasMatrixTransform = svgedit.math.hasMatrixTransform,
        da = e.transformListToTransform = svgedit.math.transformListToTransform, Ya = svgedit.math.snapToAngle,
    ...
    var sb = e.getUrlFromAttr = svgedit.utilities.getUrlFromAttr, ib = e.getHref = svgedit.utilities.getHref,
        lb = e.setHref = svgedit.utilities.setHref, Tb = svgedit.utilities.getPathBBox;
    e.getBBox = svgedit.utilities.getBBox;
    var bb = e.getRotationAngle = svgedit.utilities.getRotationAngle, va = e.getElem = svgedit.utilities.getElem,
        Ga = e.assignAttributes = svgedit.utilities.assignAttributes,
        Oa = this.cleanupElement = svgedit.utilities.cleanupElement, hb = svgedit.sanitize.getNSMap(),
        ja = e.sanitizeSvg =
            svgedit.sanitize.sanitizeSvg, Ca = svgedit.history.MoveElementCommand,
        Ha = svgedit.history.InsertElementCommand, Pa = svgedit.history.RemoveElementCommand,
        Ia = svgedit.history.ChangeElementCommand, ua = svgedit.history.BatchCommand;
    e.undoMgr = new svgedit.history.UndoManager({
        handleHistoryEvent: function (b, c) {
            var d = svgedit.history.HistoryEventTypes;
            if (b == d.BEFORE_UNAPPLY || b == d.BEFORE_APPLY) e.clearSelection(); else if (b == d.AFTER_APPLY || b == d.AFTER_UNAPPLY) {
                var m = c.elements();
                e.pathActions.clear();
                ha("changed", m);
                m =
                    c.type();
                d = b == d.AFTER_APPLY;
                if (m == Ca.type()) (d ? c.newParent : c.oldParent) == n && e.identifyLayers(); else if (m == Ha.type() || m == Pa.type()) {
                    c.parent == n && e.identifyLayers();
                    if (m == Ha.type()) d && tb(c.elem); else d || tb(c.elem);
    ...
    svgedit.utilities.snapToGrid = function (b) {
        var c = v.snappingStep, d = v.baseUnit;
        if (d !== "px") c *= svgedit.units.getTypeMap()[d];
        return b = Math.round(b /
                c) * c
    };
    var la = svgedit.utilities.snapToGrid, ub = {
            exportNoBlur: "Blurred elements will appear as un-blurred",
            exportNoforeignObject: "foreignObject elements will not appear",
            exportNoDashArray: "Strokes will appear filled",
            exportNoText: "Text may not appear as expected"
        }, Hb = ["clip-path", "fill", "filter", "marker-end", "marker-mid", "marker-start", "mask", "stroke"], Ab = $.data,
        Ib = document.createElementNS(s, "animate");
    $(Ib).attr({attributeName: "opacity", begin: "indefinite", dur: 1, fill: "freeze"}).appendTo(k);
    var tb = function (b) {
            var c =
                $(b).attr(Hb), d;
            for (d in c) {
        ...
        }, Jb = {}, Bb = v.imgPath + "logo.png", Kb = [], Na = {round_digits: 5}, xa = false, Qa = null, sa = "select",
        cb = "none", pb = {}, Ua = Z.text, Sa = N, La = null, wa = null, jb = [], Cb = {}, Vb = null, Ub = {};
    e.clipBoard = [];
    ...
    this.addExtension = function (b, c) {
        if (b in Cb) console.log('Cannot add extension "' + b + '", an extension by that name already exists"'); else {
    ...
    getStrokedBBox = this.getStrokedBBox = function (b) {
        ...
        var c = function (w) {
            try {
                var u = svgedit.utilities.getBBox(w), t = svgedit.utilities.getRotationAngle(w);
                if (t && t % 90 || svgedit.math.hasMatrixTransform(svgedit.transformlist.getTransformList(w))) {
                    t =
                        false;
                    if (["ellipse", "path", "line", "polyline", "polygon"].indexOf(w.tagName) >= 0) u = t = e.convertToPath(w, true); else if (w.tagName == "rect") {
                        var C = w.getAttribute("rx"), E = w.getAttribute("ry");
                        if (C || E) u = t = e.convertToPath(w, true)
                    }
                    if (!t) {
                        var H = w.cloneNode(true), I = document.createElementNS(s, "g"), P = w.parentNode;
                        P.appendChild(I);
                        I.appendChild(H);
        ...
        var m = d.x + d.width, i = d.y + d.height, z = d.x, o = d.y, h = function (w) {
            var u = w.getAttribute("stroke-width"), t = 0;
    ...
    var Lb = this.getVisibleElements = function (b) {
        ...
        $(b).children().each(function (d, m) {
            try {
                m.getBBox() && c.push({elem: m, bbox: getStrokedBBox([m])})
            } catch (i) {
            }
        });
        return c.reverse()
        ...
        if ($(b).data("gsvg")) $(c).data("gsvg", c.firstChild); else if ($(b).data("symbol")) {
            b = $(b).data("symbol");
            $(c).data("ref",
                b).data("symbol", b)
    ...
    this.setRotationAngle = function (b, c) {
        b = parseFloat(b);
        var d = L[0], m = d.getAttribute("transform"), i = svgedit.utilities.getBBox(d), z = i.x + i.width / 2,
            o = i.y + i.height / 2;
        i = ca(d);
    ...
    var Ob = this.recalculateAllSelectedDimensions = function () {
        ...
        kc = function (b) {
            ...
            var m = v.gridSnapping && b.parentNode.parentNode.localName === "svg", i = function () {
                if (m)for (var t in c)c[t] = la(c[t]);
                Ga(b, c, 1E3, true)
            };
            box = svgedit.utilities.getBBox(b);
            for (var z = 0; z < 2; z++) {
                var o = z === 0 ? "fill" : "stroke", h = b.getAttribute(o);
                if (h && h.indexOf("url(") === 0)if (d.a < 0 || d.d < 0) {
                    h = rb(h).cloneNode(true);
                    if (d.a < 0) {
                        var q =
                            h.getAttribute("x1"), w = h.getAttribute("x2");
            ...
            b = rb(b).firstChild;
            var m = ca(b), i = k.createSVGTransform();
            i.setTranslate(c, d);
            m.appendItem(i);
            Ta(b)
            ...
            if (c && c.numberOfItems > 0) {
                for (var d = c.numberOfItems; d--;) {
                    var m = c.getItem(d);
                    if (m.type ===
                        0) c.removeItem(d); else if (m.type === 1) svgedit.math.isIdentity(m.matrix) && c.removeItem(d); else m.type === 4 && m.angle === 0 && c.removeItem(d)
                }
            ...
            if (c) {
                ...
                for (var i = []; d--;) {
                    m = c.getItem(d);
                    if (m.type === 1) i.push([m.matrix, d]); else if (i.length) i = []
                }
                if (i.length === 2) {
            ...
            } else if (z) o = {x: $(z).attr("x") || 0, y: $(z).attr("y") || 0};
            if (h == null) {
                h = $.extend(true, {}, o);
            ...
            if (b.tagName == "g" && !z || b.tagName == "a") {
                ...
                var t = {x: i.x + i.width / 2, y: i.y + i.height / 2},
                ...
                if (E >= 3 && c.getItem(E - 2).type == 3 && c.getItem(E - 3).type == 2 && c.getItem(E - 1).type == 2) {
                    w = 3;
                    var I = c.getItem(E - 3).matrix, P = c.getItem(E - 2).matrix, W = c.getItem(E - 1).matrix;
                    q = b.childNodes;
                    for (u = q.length; u--;) {
                        var Y = q.item(u);
                        m = z = 0;
                        if (Y.nodeType == 1) {
                            ...
                            if (R) {
                                ...
                                if (z || ia(R)) {
                                    ...
                                    var Ja = k.createSVGTransform(), Za = k.createSVGTransform(),
            ...
            return d
        }, nb = null, za = this.clearSelection = function (b) {
            ...
            if (b.length != 0) {
                ...
                for (L.sort(function (z, o) {
                    if (z && o && z.compareDocumentPosition)return 3 -
                        (o.compareDocumentPosition(z) & 6); else if (z == null)return 1
                }); L[0] == null;)L.shift(0)
            }
    ...
    (function () {
        var b = null, c = null, d = null, m =
            null, i = null, z = {}, o = {minx: null, miny: null, maxx: null, maxy: null};
        $(a).mousedown(function (h) {
            if (!(e.spaceKey || h.button === 1)) {
                var q = h.button === 2;
                h.altKey && svgCanvas.cloneSelectedElements(0, 0);
                nb = n.getScreenCTM().inverse();
                var w = oa(h.pageX, h.pageY, nb), u = w.x * A, t = w.y * A;
                h.preventDefault();
                if (q) {
                    sa = "select";
                    Vb = w
                ...
                if (C == ra.selectorParentGroup && L[0] != null) {
                    C =
                        h.target;
                    var H = Ab(C, "type");
                    if (H == "rotate") sa = "rotate"; else if (H == "resize") {
                        sa = "resize";
                        cb = Ab(C, "dir")
                    }
                    C = L[0]
                ...
                switch (sa) {
                    case "select":
                        ...
                        if (C != k) {
                            ...
                            m *= A;
                            i *= A;
                            Ga(wa, {x: m, y: i, width: 0, height: 0, display: "inline"}, 100)
                        }
                        break;
                    case "zoom":
                        xa = true;
                        if (wa == null) wa = ra.getRubberBandBox();
                        Ga(wa, {x: u * A, y: u * A, width: 0, height: 0, display: "inline"}, 100);
                        break;
                    case "resize":
                        xa = true;
                        c = w;
                    ...
                    case "rect":
                        ...
                        na({
                            element: "rect", curStyles: true, attr: {
                                x: w, y: t, width: 0,
                                height: 0, id: Fa(), opacity: N.opacity / 2
                            }
                        });
                        break;
                    case "line":
                    ...
                    case "ellipse":
                        ...
                        na({
                            element: "ellipse",
                            curStyles: true,
                            attr: {cx: w, cy: t, rx: 0, ry: 0, id: Fa(), opacity: N.opacity / 2}
                        });
                        break;
                    case "text":
                        xa = true;
                ...
                h = Db("mouseDown", {event: h, start_x: c, start_y: d, selectedElements: L}, true);
                $.each(h, function (W, Y) {
            ...
            if (xa)if (!(h.button === 1 || e.spaceKey)) {
                var q = L[0], w = oa(h.pageX, h.pageY, nb), u = w.x * A;
                w = w.y * A;
                var t = va(db()), C = x = u / A, E = y = w / A;
                if (v.gridSnapping) {
                    x = la(x);
                    y = la(y)
                }
                ...
                switch (sa) {
                    ...
                    case "multiselect":
                        ...
                        for (E = 0; E < I; ++E) {
                            H = P.indexOf(L[E]);
                            if (H == -1) t.push(L[E]); else P[H] = null
                        }
                        I = P.length;
                        for (E = 0; E < I; ++E)P[E] &&
                        C.push(P[E]);
                    ...
                    case "resize":
                        ...
                        var V = T = 0, aa = Y ? (Y + H) / Y : 1, Ja = W ? (W + C) / W : 1;
                        if (cb.indexOf("n") >=
                            0) {
                            aa = Y ? (Y - H) / Y : 1;
                            V = Y
                        ...
                        if (h.shiftKey)if (Ja == 1) Ja = aa; else aa = Ja;
                        H.setScale(Ja, aa);
                    ...
                    case "image":
                        ...
                        Ga(t, {width: C, height: I, x: E, y: P}, 1E3);
                        break;
                    case "circle":
                        I = $(t).attr(["cx", "cy"]);
                        C = I.cx;
                ...
                Db("mouseMove", {event: h, mouse_x: u, mouse_y: w, selected: q})
            }
        }).click(function (h) {
            h.preventDefault();
            return false
            ...
            if (h.button !==
                ...
                La = null;
                if (xa) {
                    var w = oa(h.pageX, h.pageY, nb), u = w.x * A;
                    w = w.y * A;
                    var t = u / A, C = w / A, E = va(db()), H = false;
                    xa = false;
                    switch (sa) {
                        case "resize":
                        case "multiselect":
                        ...
                        case "select":
                            if (L[0] != null) {
                                ...
                                if (t != m || C != i) {
                                    ...
                                    u = h.target;
                                    if (L[0].nodeName ===
                                        "path" && L[1] == null) qa.select(L[0]); else h.shiftKey && q != u && e.removeFromSelection([u])
                                }
                        ...
                        case "fhellipse":
                            if (o.maxx - o.minx > 0 && o.maxy - o.miny > 0) {
                                E = na({
                                    element: "ellipse", curStyles: true,
                                    attr: {
                                        cx: (o.minx + o.maxx) / 2,
                                        cy: (o.miny + o.maxy) / 2,
                                        rx: (o.maxx - o.minx) / 2,
                    ...
                    u = Db("mouseUp", {event: h, mouse_x: u, mouse_y: w}, true);
                    $.each(u, function (W, Y) {
                        if (Y) {
                            H = Y.keep || H;
                            E = Y.element;
                    ...
                    if (!H && E != null) {
                        ...
                        if (Ib.beginElement && E.getAttribute("opacity") != N.opacity) {
                            I = $(Ib).clone().attr({to: N.opacity, dur: h}).appendTo(E);
                            try {
                                I[0].beginElement()
                            } catch (P) {
                            }
                        ...
                        setTimeout(function () {
                            ...
                            Oa(E);
                            if (sa === "path") qa.toEditMode(E); else v.selectNew && Wa([E], true);
                            ma(new Ha(E));
                            ha("changed", [E])
                        }, h * 1E3)
                    }
        ...
        $(a).bind("mousewheel DOMMouseScroll",
            function (h) {
                if (h.shiftKey) {
                    ...
                    q = {x: q.x, y: q.y, width: 0, height: 0};
                    if (h.wheelDelta)if (h.wheelDelta >= 120) q.factor = 2; else {
                        if (h.wheelDelta <= -120) q.factor = 0.5
                    } else if (h.detail)if (h.detail > 0) q.factor = 0.5; else if (h.detail < 0) q.factor = 2;
    ...
    var Mb = function (b) {
        ...
        function b(R) {
            var T = w.value === "";
            $(w).focus();
            if (!arguments.length)if (T) R = 0; else {
                if (w.selectionEnd !==
            ...
            if (!u) {
                ...
                Ga(u, {id: "text_cursor", stroke: "#333", "stroke-width": 1});
            ...
            Ga(u, {x1: T.x, y1: T.y, x2: V.x, y2: V.y, visibility: "visible", display: "inline"});
            t && t.setAttribute("d", "")
        }
        ...
        function c(R, T, V) {
            if (R === T) b(T); else {
                ...
                if (!t) {
                    ...
                    Ga(t, {id: "text_selectblock", fill: "green", opacity: 0.5, style: "pointer-events:none"});
        ...
        function h(R) {
            if (Y && q) {
                ...
                T = i(T.x * A, T.y * A);
                T = d(T.x, T.y);
                var V = q.textContent, aa = V.substr(0, T).replace(/[a-z0-9]+$/i, "").length;
        ...
        return {
            select: function (R, T, V) {
            ...
            }, setCursor: b, toEditMode: function (R, T) {
                Y = false;
                sa = "textedit";
                ...
                if (q) {
                    ...
                    var R = q.textContent.length, T = q.getAttribute("transform");
                    H = svgedit.utilities.getBBox(q);
                    I = T ? kb(q) : null;
                    E = Array(R);
                    w.focus();
                    $(q).unbind("dblclick", h).dblclick(h);
                    if (!R)var V = {x: H.x + H.width / 2, width: 0};
                    for (T = 0; T < R; T++) {
                        ...
                        E[T] = {x: aa.x, y: H.y, width: V.x - aa.x, height: H.height}
                    }
                    E.push({x: V.x, width: 0});
        ...
        svgedit.path.Path.prototype.endChanges = function (o) {
            ...
            o = new Ia(this.elem, {d: this.last_d}, o);
            ma(o);
            ...
            mouseDown: function (o, h, q, w) {
                if (sa === "path") {
                    ...
                    if (!q) {
                        ...
                        Ga(q, {id: "path_stretch_line", stroke: "#22C", "stroke-width": "0.5", fill: "none"});
                    ...
                    if (i) {
                        ...
                        d_attr = "M" + w + "," + h + " ";
                        i = na({element: "path", curStyles: true, attr: {d: d_attr, id: Fa(), opacity: N.opacity / 2}});
                        q.setAttribute("d",
                    ...
                    if (C.substr(0, 14) == "pathpointgrip_") {
                        ...
                        if (o.shiftKey) u.selected ? svgedit.path.path.removePtFromSelection(h) : svgedit.path.path.addPtsToSelection(h); else {
                    ...
                    if (!svgedit.path.path.dragging) {
                        if (wa == null) wa = ra.getRubberBandBox();
                        Ga(wa, {x: q * A, y: w * A, width: 0, height: 0, display: "inline"}, 100)
                    }
                }
                ...
                if (sa === "path") {
                    if (i) {
                        var q = i.pathSegList, w = q.numberOfItems - 1;
                        if (d) {
                            ...
                            w = q.getItem(w);
                            if (w.pathSegType === 6) svgedit.path.replacePathSeg(6, 1, [o, h, (w.x + (w.x - w.x2)) * A, (w.y + (w.y - w.y2)) * A, o, h], u); else m ? svgedit.path.replacePathSeg(6, 1, [o, h, m[0], m[1], o, h], u) : svgedit.path.replacePathSeg(4, 1, [o, h], u)
                        }
                    }
                    ...
                    C = svgedit.path.getPointFromGrip({x: o, y: h}, svgedit.path.path);
                    w = C.x - u.x;
                    u = C.y -
                        u.y;
                    ...
                    svgedit.path.path.eachSeg(function () {
                        if (this.next || this.prev) {
                            var P = wa.getBBox(), W = svgedit.path.getGripPt(this);
                            P = svgedit.math.rectsIntersect(P, {x: W.x, y: W.y, width: 0, height: 0});
                            this.select(P);
                ...
                if (o)if (bb(o) != 0) {
                    var h = new ua("Reorient path"),
                        q = {d: o.getAttribute("d"), transform: o.getAttribute("transform")};
                ...
                return {x: o.item.x, y: o.item.y, type: o.type}
                ...
                if (o.length === 1) {
                    var h = svgedit.path.path.elem, q = h.pathSegList, w = o[0], u = null, t = null;
                    ...
                    if (u !== false) {
                        var C = h.createSVGPathSegLinetoAbs(t.x, t.y), E = h.createSVGPathSegClosePath();
                        ...
                        for (o = 0; o < q.numberOfItems; o++) {
                            var H = q.getItem(o);
                            if (H.pathSegType === 2) C = o; else if (o === w) q.removeItem(C); else if (H.pathSegType === 1 && w < o) {
                                E = o - 1;
                                q.removeItem(o);
                                break
                            }
                ...
                if (qa.canDeleteNodes) {
                    ...
                    var q = function () {
                        var w = svgedit.path.path.elem.pathSegList, u = w.numberOfItems, t = function (H, I) {
                ...
                var h = o.points, q = h.numberOfItems;
                if (q >= 4) {
                    var w = h.getItem(0), u = null;
                    o = [];
                    o.push(["M", w.x, ",", w.y, " C"].join(""));
                    for (var t = 1; t <= q - 4; t += 3) {
                        var C = h.getItem(t), E = h.getItem(t + 1), H = h.getItem(t + 2);
                    ...
                    o = o.join(" ");
                    o = na({element: "path", curStyles: true, attr: {id: db(), d: o, fill: "none"}})
                }
                return o
                ...
                for (var q = o.pathSegList, w = q.numberOfItems, u = 0, t = 0, C = "", E = null, H = 0; H < w; ++H) {
                    var I = q.getItem(H), P = I.x || 0, W = I.y || 0, Y = I.x1 || 0, R = I.y1 || 0, T = I.x2 || 0,
                        V = I.y2 || 0, aa = I.pathSegType, Ja = Xb[aa]["to" + (h ? "Lower" : "Upper") + "Case"](),
                    ...
                    switch (aa) {
                        ...
                        case 11:
                            ...
                            Za([[I.r1, I.r2]], [I.angle, I.largeArcFlag ? 1 : 0, I.sweepFlag ? 1 : 0], [P, W]);
                            break;
                        case 16:
                            P -= u;
                            T -= u;
        ...
        for (var c = [], d = 0, m = ["fill", "stroke", "filter", "marker-start", "marker-mid", "marker-end"],
                 i = m.length, z = n.getElementsByTagNameNS(s, "*"), o = z.length, h = 0; h < o; h++) {
    ...
    this.svgCanvasToString = function () {
        ...
        $(n).find("g:data(gsvg)").each(function () {
            for (var d = this.attributes, m = d.length,
                     i = 0; i < m; i++)if (d[i].nodeName == "id" || d[i].nodeName == "style") m--;
            if (m <= 0) {
                d = this.firstChild;
                b.push(d);
                $(this).replaceWith(d)
    ...
    this.svgToString = function (b, c) {
        var d = [], m = svgedit.utilities.toXml, i = v.baseUnit, z = RegExp("^-?[\\d\\.]+" + i + "$");
        if (b) {
            Oa(b);
            var o = b.attributes, h, q, w = b.childNodes;
    ...
    var Rb = this.uniquifyElems = function (b) {
        ...
        svgedit.utilities.walkTree(b, function (h) {
            if (h.nodeType == 1) {
                if (h.id) {
                    h.id in c || (c[h.id] = {elem: null, attrs: [], hrefs: []});
                    c[h.id].elem = h
                }
                $.each(Hb, function (w, u) {
                    ...
                    if (t) {
                        ...
                        if (C = C ? C.substr(1) : null) {
                            C in c || (c[C] = {elem: null, attrs: [], hrefs: []});
                            c[C].attrs.push(t)
                        }
                    }
                });
                ...
                if (q && d.indexOf(h.nodeName) >= 0)if (q = q.substr(1)) {
                    q in c || (c[q] = {elem: null, attrs: [], hrefs: []});
                    c[q].hrefs.push(h)
                }
            }
        });
        ...
        c.each(function () {
            if ($(this).attr("gradientUnits") === "userSpaceOnUse") {
                ...
                if (d.length)if (d = svgedit.utilities.getBBox(d[0]))if (this.tagName === "linearGradient") {
                    var m = $(this).attr(["x1", "y1", "x2", "y2"]), i = this.gradientTransform.baseVal;
                    ...
                    $(this).attr({
                        x1: (m.x1 - d.x) / d.width,
                        y1: (m.y1 - d.y) / d.height, x2: (m.x2 - d.x) / d.width, y2: (m.y2 - d.y) / d.height
                    });
    ...
    this.setSvgString = function (b) {
        try {
            ...
            this.prepareSvg(c);
            var d = new ua("Change Source"), m = n.nextSibling, i = k.removeChild(n);
            ...
            var h = {id: "svgcontent", overflow: v.show_outside_canvas ? "visible" : "hidden"}, q = false;
            if (z.attr("viewBox")) {
                ...
                var I = z.attr(H);
                I || (I = "100%");
                if ((I + "").substr(-1) === "%") q = true; else h[H] = ba(H, I)
            });
            zb();
    ...
    this.importSvgString = function (b) {
        try {
            var c =
                svgedit.utilities.encode64(b.length + b).substr(0, 32), d = false;
            if (pb[c])if ($(pb[c].symbol).parents("#svgroot").length) d = true;
            var m = new ua("Import SVG");
            if (d)var i = pb[c].symbol, z = pb[c].xform; else {
                var o = svgedit.utilities.text2xml(b);
                this.prepareSvg(o);
                var h;
                h = f.adoptNode ? f.adoptNode(o.documentElement) : f.importNode(o.documentElement, true);
                Rb(h);
                var q = ba("width", h.getAttribute("width")), w = ba("height", h.getAttribute("height")),
                    u = h.getAttribute("viewBox"), t = u ? u.split(" ") : [0, 0, q, w];
    ...
    this.cloneLayer = function (b) {
        var c =
            new ua("Duplicate Layer"), d = f.createElementNS(s, "g"), m = f.createElementNS(s, "title");
        m.textContent = b;
        d.appendChild(m);
        m = B().getCurrentLayer();
        $(m).after(d);
    ...
    this.deleteCurrentLayer = function () {
        var b = B().getCurrentLayer(), c = b.nextSibling, d = b.parentNode;
            ...
            if (!e.setCurrentLayer(b)) {
                ...
                for (i = 0; i < o; ++i) {
                    ...
                    if (h && h.tagName == "title") {
                        ...
                        h.textContent = b;
                        m.addSubCommand(new Ia(h, {"#text": z}));
                        ma(m);
                        ha("changed", [d]);
                        return true
                    }
    ...
    this.setCurrentLayerPosition = function (b) {
        ...
        if (c.current_layer && b >= 0 && b < c.getNumLayers()) {
            ...
            if (d != b) {
                var m = null, i = c.current_layer.nextSibling;
                if (b > d) {
    ...
    this.setLayerVisibility = function (b, c) {
        var d = B(), m = d.getLayerVisibility(b), i = d.setLayerVisibility(b, c);
        if (i) ma(new Ia(i, {display: m ? "inline" : "none"}, "Layer Visibility")); else return false;
    ...
    this.moveSelectedToLayer = function (b) {
        for (var c = null, d = B(), m = 0; m < d.getNumLayers(); ++m)if (d.getLayerName(m) ==
            b) {
            c = d.all_layers[m][1];
            break
        }
        ...
        for (m = d.length; m--;) {
            var i = d[m];
            if (i) {
                var z = i.nextSibling, o = i.parentNode;
                c.appendChild(i);
    ...
    this.mergeLayer = function (b) {
        var c = new ua("Merge Layer"), d = B(), m = $(d.current_layer).prev()[0];
        if (m) {
    ...
    this.mergeAllLayers = function () {
        var b = new ua("Merge all Layers"), c = B();
    ...
    var xb = this.getResolution = function () {
        var b = n.getAttribute("width") /
            A, c = n.getAttribute("height") / A;
        return {w: b, h: c, zoom: A}
    };
    ...
    this.setGroupTitle = function (b) {
        ...
        c = $(c).data("gsvg") ||
            c;
        var d = $(c).children("title"), m = new ua("Set Label");
        if (b.length)if (d.length) {
            d = d[0];
            m.addSubCommand(new Ia(d, {"#text": d.textContent}));
            d.textContent = b
        } else {
    ...
    this.setDocumentTitle = function (b) {
        for (var c = n.childNodes, d = false, m = "", i = new ua("Change Image Title"),
                 z = 0; z < c.length; z++)if (c[z].nodeName ==
            "title") {
            d = c[z];
            m = d.textContent;
            break
        ...
        if (b.length) d.textContent = b; else d.parentNode.removeChild(d);
        i.addSubCommand(new Ia(d, {"#text": m}));
        ma(i)
    };
    ...
    this.setResolution = function (b, c) {
        ...
        if (b != m || c != d) {
            ...
            this.contentW = b;
            this.contentH = c;
            i.addSubCommand(new Ia(n, {width: m, height: d}));
    ...
    this.setColor = function (b, c, d) {
        N[b] = c;
        Sa[b + "_paint"] = {type: "solidColor"};
        for (var m = [], i = L.length; i--;) {
            var z = L[i];
            if (z)if (z.tagName == "g") svgedit.utilities.walkTree(z, function (o) {
                o.nodeName != "g" && m.push(o)
            }); else if (b == "fill") z.tagName != "polyline" && z.tagName != "line" && m.push(z); else m.push(z)
        }
        if (m.length > 0)if (d) ob(b, c, m); else {
            Va(b, c, m);
            ha("changed", m)
        }
    };
    var Xa = function () {
        ...
        for (var d = c.length, m = ["r", "cx", "cy", "fx", "fy"]; d--;) {
            ...
            if (b.tagName ==
                ...
                var z = $(b).attr(m), o = $(i).attr(m), h = false;
                $.each(m, function (E, H) {
                    if (z[H] != o[H]) h = true
                });
                if (h)continue
            }
            var q = b.getElementsByTagNameNS(s, "stop"), w = i.getElementsByTagNameNS(s, "stop");
    ...
    this.setStrokeWidth = function (b) {
        if (b == 0 && ["line", "path"].indexOf(sa) >= 0) e.setStrokeWidth(1); else {
            Sa.stroke_width = b;
        ...
        e.setBlur = function (i, z) {
            if (c) b(); else {
                ...
                if (d) {
                    ...
                    var w = na({element: "feGaussianBlur", attr: {"in": "SourceGraphic", stdDeviation: i}});
                    d = na({element: "filter", attr: {id: h + "_blur"}});
                    d.appendChild(w);
                    Xa().appendChild(d);
                    q.addSubCommand(new Ha(d))
                }
                w = {filter: o.getAttribute("filter")};
                if (i === 0) {
    ...
    this.setImageURL = function (b) {
        ...
        if (c) {
            ...
            var m = ib(c);
            if (m !== b) d =
                true; else if (!d)return;
            var i = new ua("Change Image URL");
            lb(c, b);
            i.addSubCommand(new Ia(c, {"#href": m}));
    ...
    this.setLinkURL = function (b) {
        ...
        if (c) {
            ...
            if (d !== b) {
                ...
                lb(c,
                    b);
                m.addSubCommand(new Ia(c, {"#href": d}));
                ma(m)
            }
        }
    };
    ...
    this.convertToPath = function (b, c) {
        if (b == null) $.each(L, function (Y, R) {
            ...
            switch (b.tagName) {
                case "ellipse":
                case "circle":
                    var t = $(b).attr(["rx", "ry", "cx", "cy"]), C = t.cx, E = t.cy, H = t.rx;
                    t = t.ry;
                    if (b.tagName == "circle") H = t = $(b).attr("r");
                    w([["M", [C - H, E]], ["C", [C - H, E - t / u, C - H / u, E - t, C, E - t]], ["C", [C + H / u, E -
                    t, C + H, E - t / u, C + H, E]], ["C", [C + H, E + t / u, C + H / u, E + t, C, E + t]], ["C", [C - H / u, E + t, C - H, E + t / u, C - H, E]], ["Z", []]]);
                    break;
                case "path":
                    q = b.getAttribute("d");
                    break;
                ...
                case "rect":
                    ...
                    I = I.height;
                    u = 4 - u;
                    !H && !t ? w([["M", [C, E]], ["L", [C + P, E]], ["L", [C + P, E + I]], ["L", [C, E + I]], ["L", [C, E]], ["Z", []]]) :
                        w([["M", [C, E + t]], ["C", [C, E + t / u, C + H / u, E, C + H, E]], ["L", [C + P - H, E]], ["C", [C + P - H / u, E, C + P, E + t / u, C + P, E + t]], ["L", [C + P, E + I - t]], ["C", [C + P, E + I - t / u, C + P - H / u, E + I, C + P - H, E + I]], ["L", [C + H, E + I]], ["C", [C + H / u, E + I, C, E + I - t / u, C, E + I - t]], ["L", [C, E + t]], ["Z", []]]);
                    break;
                default:
            ...
            if (h != null) {
                ...
                if ((b === "x" || b === "y") && z.indexOf(h.tagName) >= 0) {
                    ...
                    if (q !== String(c)) {
                        ...
                        q = bb(h);
                        if (q != 0 && b != "transform")for (var w = ca(h),
                                                                u = w.numberOfItems; u--;)if (w.getItem(u).type == 4) {
                            w.removeItem(u);
    ...
    this.pasteElements = function (b, c, d) {
        ...
        if (i) {
            ...
            if (b !== "in_place") {
                ...
                b = getStrokedBBox(z);
                var t = w - (b.x + b.width / 2), C = u - (b.y + b.height / 2), E = [], H = [];
                $.each(z, function () {
                    E.push(t);
                    H.push(C)
                });
    ...
    var Yb = this.pushGroupProperties = function (b, c) {
        var d = b.childNodes, m = d.length, i = b.getAttribute("transform"), z = ca(b), o = da(z).matrix,
            h = new ua("Push group properties"),
            q = 0, w = bb(b), u = $(b).attr(["filter", "opacity"]), t, C;
        for (q = 0; q < m; q++) {
            var E = d[q];
            if (E.nodeType === 1) {
                ...
                if (u.filter) {
                    ...
                    C || (C = this.getBlur(b));
                    if (H) H = C - 0 + (H - 0); else if (H === 0) H = C;
                    if (I) t = rb(E.getAttribute("filter")); else if (t) {
                        t = vb(t);
                        Xa().appendChild(t)
                    } else t = rb(u.filter);
                ...
                if (H)if (E.tagName !== "defs")if (z.numberOfItems) {
                    if (w && z.numberOfItems == 1) {
                        var P = z.getItem(0).matrix, W = k.createSVGMatrix();
                        if (I = bb(E)) W = H.getItem(0).matrix;
                        var Y = svgedit.utilities.getBBox(E), R = da(H).matrix,
    ...
    this.ungroupSelectedElement =
        function () {
            var b = L[0];
            if ($(b).data("gsvg") || $(b).data("symbol")) gc(b); else if (b.tagName === "use") {
                ...
                if (b.tagName === "g" || b.tagName === "a") {
                    ...
                    for (var m = b.nextSibling, i = Array(b.childNodes.length), z = 0; b.firstChild;) {
                        var o = b.firstChild, h = o.nextSibling, q = o.parentNode;
                        if (o.tagName === "title") {
    ...
    this.moveToBottomSelectedElement = function () {
        ...
        if (b != null) {
            b = b;
            var c = b.parentNode, d = b.nextSibling, m = b.parentNode.firstChild;
    ...
    this.moveSelectedElements = function (b, c, d) {
        ...
        for (var m = new ua("position"), i = L.length; i--;) {
            var z = L[i];
            if (z != null) {
                var o = k.createSVGTransform(), h = ca(z);
    ...
    this.alignSelectedElements = function (b, c) {
        var d = [], m = Number.MAX_VALUE, i = Number.MIN_VALUE, z = Number.MAX_VALUE, o = Number.MIN_VALUE,
            h = Number.MIN_VALUE, q = Number.MIN_VALUE, w = L.length;
        if (w) {
            for (var u = 0; u < w; ++u) {
                if (L[u] ==
                    null)break;
    ...
    this.updateCanvas = function (b, c) {
        ...
        var d = $("#canvasBackground")[0], m = n.getAttribute("x"), i = n.getAttribute("y"),
            z = b / 2 - this.contentW * A / 2, o = c / 2 - this.contentH * A / 2;
        Ga(n, {
        ...
        Ga(d, {width: n.getAttribute("width"), height: n.getAttribute("height"), x: z, y: o});
        (d = va("background_image")) && Ga(d, {width: "100%", height: "100%"});
        ra.selectorParentGroup.setAttribute("transform", "translate(" + z + "," + o + ")");
        return {x: z, y: o, old_x: m, old_y: i, d_x: z - m, d_y: o - i}
    };
    this.setBackground = function (b, c) {
        var d = va("canvasBackground"), m = $(d).find("rect")[0], i = va("background_image");
    ...
    this.cycleElement = function (b) {
        var c =
            L[0], d = false, m = Lb(O || B().getCurrentLayer());
        if (m.length) {
            if (c == null) {
                b = b ? m.length - 1 : 0;
                d = m[b]
                ...
                b = b ? i - 1 : i + 1;
                if (b >= m.length) b = 0; else if (b < 0) b = m.length - 1;
                d = m[b];
                break
            }
            Wa([d], true);
...
(function () {
    ...
    if (!window.svgEditor) window.svgEditor = function (a) {
        ...
        var l, s = {}, v = false, G = {lang: "en", iconsize: "m", bkgd_color: "#FFF", bkgd_url: "", img_save: "embed"},
            e = {}, f = {
                canvasName: "default",
                canvas_expansion: 3,
                dimensions: [env.ViewBoxWidth, env.ViewBoxHeight],
            ...
            k = s.uiStrings = {
                common: {
                    ...
                    key_del: "Del"
                }, layers: {layer: "图层"}, notification: {
                    "invalidAttrValGiven" : "无效的值",
                    "noContentToFitTo" : "不适合的内容",
                    "dupeLayerName" : "图层名称已存在！",
                    "enterUniqueLayerName" : "请输入一个不重复的图层名称（不超过6个字符）",
                    "enterNewLayerName" : "请输入图层的新名称（不超过6个字符）",
                    "layerHasThatName" : "图层名称已存在",
                    "QmoveElemsToLayer" : "确定移动选定的元素到“%s”吗？",
                    "QwantToClear" : "你想清除图纸吗？\n这个操作也会删除你的撤消历史！",
                    "QwantToOpen" : "你想打开一个新文件吗？\n这个操作也会删除你的撤消历史！",
                    "QerrorsRevertToSource" : "SVG源代码分析错误。\n需要回复到原来的SVG源代码吗？",
                    "QignoreSourceChanges" : "忽略SVG源代码的变化吗？",
                    "featNotSupported" : "不支持",
                    "enterNewImgURL" : "请输入新的图片URL",
                    "defsFailOnSave" : "注意：\n由于你的浏览器有个bug,图像可能显示错误（失踪的梯度或元素）。不过这将出现一次救了正确的。",
                    "loadingImage" : "正在加载图片,请稍候……",
                    "saveFromBrowser" : "在你的浏览器中选择“另存为……”保存该图片为%s文件。",
                    "noteTheseIssues" : "还需要注意以下问题",
                    "unsavedChanges" : "修改未保存！",
                    "enterNewLinkURL" : "请输入新的URL",
                    "errorLoadingSVG" : "错误：\n无法加载SVG数据",
                    "URLloadFail" : "无法加载URL",
                    "retrieving" : "正在还原“%s”……"
                }
        ...
        a.pref = function (B, A) {
            ...
            var O = location.hostname,
                Z = O && O.indexOf(".") >= 0, N = A != undefined, L = false;
            try {
            ...
            if (L)if (N) L.setItem(B, A); else {
                if (L.getItem(B))return L.getItem(B) + ""
            } else if (window.widget)if (N) widget.setPreferenceForKey(A, B); else return widget.preferenceForKey(B); else if (N) {
                O = new Date;
        ...
        s.init = function () {
            ...
            function N() {
                ...
                a.prompt(k.notification.enterUniqueLayerName, g, function (p) {
                    if (p)if (l.getCurrentDrawing().hasLayer(p)) a.alert(k.notification.dupeLayerName); else {
                        l.cloneLayer(p);
                        Na();
                        $a()
                    }
            ...
            function L(g) {
                var p = a("#layerlist tr.layersel").index(), D = l.getCurrentDrawing().getNumLayers();
                if (p > 0 || p < D - 1) {
                    p += g;
            ...
            function na(g, p) {
                ...
                for (var D = l.getContentElem(), J = svgedit.units.getTypeMap()[f.baseUnit],
                         M = 0; M < 2; M++) {
                    var Q = M === 0, U = Q ? "x" : "y", X = Q ? "width" : "height", ga = D.getAttribute(U) - 0;
                    ...
                    var ya = 0, Da, S = ea.getContext("2d");
                    ...
                    for (var Ka = ga / X % ea * X, Ma = Ka - Ea; Ka < ka; Ka += Ea) {
                        ...
                        Aa = (Ma - ga) / X;
                        if (ea >= 1) fa = Math.round(Aa); else {
                            fa = (ea + "").split(".")[1].length;
                            fa = Aa.toFixed(fa) - 0
                        }
                        if (fa !== 0 && fa !== 1E3 && fa % 1E3 === 0) fa = fa / 1E3 + "K";
                        if (Q) S.fillText(fa, Ka + 2, 8); else {
                            Aa = (fa + "").split("");
            ...
            (function () {
                ...
                if (a.isEmptyObject(g)) (g = window.localStorage.getItem("svgedit-" + s.curConfig.canvasName)) && s.loadFromString(g); else {
                    ...
                    var p = g.source, D = a.param.querystring();
                    if (!p)if (D.indexOf("source=data:") >= 0) p = D.match(/source=(data:[^&]*)/)[1];
                    if (p)if (p.indexOf("data:") === 0) {
                        p = p.replace(/ /g, "+");
                        s.loadFromDataURI(p)
                    } else s.loadFromString(p);
                    else if (D.indexOf("paramurl=") !== -1) svgEditor.loadFromURL(D.substr(9)); else g.url && svgEditor.loadFromURL(g.url)
                }
            })();
            var ca = function () {
            ...
            a.svgIcons(f.imgPath + "svg_edit_icons.svg",
                {
                    ...
                    placement: {
                        ...
                        '#layerlist td.layerrename' : 'text',
                        '#layerlist td.layerdelete' : 'delete',
                    ...
                    resize: {
                        ...
                        '.layer_button .svg_icon, #layerlist td.layerrename .svg_icon' : 14,
                        '.layer_button .svg_icon, #layerlist td.layerdelete .svg_icon' : 14,
                    ...
                    callback: function () {
                        ...
                        a(".tools_flyout").each(function () {
                            var p = a("#" + this.id + "_show"), D = p.attr("data-curopt");
            ...
            var oa = navigator.userAgent.indexOf("AppleWebKit") >= 0, pa = ca ? "meta+" : "ctrl+", ia = l.pathActions,
                da = l.undoMgr, Ya = svgedit.utilities, kb = f.imgPath + "logo.png", ba = a("#workarea"),
                sb = a("#cmenu_canvas");
            a("#cmenu_layers");
            var ib = null, lb = 1, Tb = "crosshair", bb = "crosshair", va = "toolbars", Ga = "",
                Oa = {fill: null, stroke: null};
            (function () {
                a("#dialog_container").draggable({cancel: "#dialog_content, #dialog_buttons *", containment: "window"});
                var g = a("#dialog_box"), p = a("#dialog_buttons"),
                    D = function (J, M, Q, U) {
                ...
                var dbox = function(type, msg, callback, defText) {
                    ...
                    $('#dialog_content')
                        ...
                        .toggleClass(
                            'prompt',
                            (type == 'prompt' || type == 'newlayer'));
                    p.empty();

                    var ok = $(
                        '<input type="button" value="'
                        + k.common.ok + '">').appendTo(
                        p);
                    ...
                    if (type == 'newlayer') {
                        ...
                        var layerinfo = $('<div style="padding-right:9px;"></div>');
                        layerinfo.prependTo(p);
                        var input = $(
                            '<input type="text" maxlength="6" style="width:150px;display:inline;margin-top:1px;margin-right:35px;"></input>')
                            .appendTo(layerinfo);
                        console.log("defText.layername="+defText.layername);
                        input.val(defText.layername || '');
                        input.bind('keydown', 'return', function() {
                            haskeydown = true;
                        });
                        input.bind('keyup', 'return', function() {
                            if (haskeydown) {
                                ok.click();
                            }
                        });
                        var layerTypeList = svgCanvas.getLayerTypeList();
                        var typeOption = "";
                        for(var layertype in env.typeJson){
                            if((!layerTypeList[layertype]) || (defText.layertype == layertype)) {
                                typeOption = typeOption+"<option id=\'typecode_"+layertype+"\' value=\'"+layertype+"\'>"+env.typeJson[layertype]+"</option>";
                            }
                        }
                        var hasType = true;
                        if(!typeOption){
                            hasType = false;
                            typeOption = "<option value=\'\'>无可用类型</option>";
                        }
                        var typeselector = $(
                            '<select id="changeLayerType" style="width:88px;">'
                            + typeOption + '</select>')
                            .appendTo(layerinfo);
                        if(!hasType){
                            typeselector.attr("disabled","disabled");
                        }
                        typeselector.val(defText.layertype);
                        typeselector.bind('keydown', 'return', function() {
                            haskeydown = true;
                        });
                        typeselector.bind('keyup', 'return', function() {
                            if (haskeydown) {
                                ok.click();
                            }
                        });
                    ...
                    if (type == 'prompt') {
                        var input = $('<input type="text">').prependTo(
                            p);
                        input.val(defText || '');
                        input.bind('keydown', 'return', function() {
                            ok.click();
                    ...
                    ok.click(
                        function() {
                            ...
                            if (type == "newlayer") {
                                ...
                                resp = {
                                    layername : layername,
                                    layertype : typeselector.val()
                                };
                                if (layername != svgedit.utilities
                                        .toXml(layername)) {
                                    resp.error = "名称不合法！";
                                }
                            } else {
                                resp = (type == 'prompt') ? input.val()
                                    : true;
                            }
                            g.hide();
                            if (callback) {
                    ...
                    if (type == 'prompt' || type == 'newlayer')
                        input.focus();
                }

                a.alert = function(msg, cb) {
                    dbox('alert', msg, cb);
                };
                a.confirm = function(msg, cb) {
                    dbox('confirm', msg, cb);
                };
                a.process_cancel = function(msg, cb) {
                    dbox('process', msg, cb);
                };
                a.prompt = function(msg, txt, cb) {
                    dbox('prompt', msg, cb, txt);
                };
                a.newlayer = function(msg, txt, cb) {
                    dbox('newlayer', msg, cb, txt);
                };
            ...
            var hb = function () {
                    ...
                    l.setMode("select");
                    ba.css("cursor", "auto")
                }, ja = null, Ca = false, Ha = false, Pa = false, Ia = false, ua = "", ma = a("title:first").text(),
                ra = l.zoomChanged = function (g, p, D) {
                    ...
                    if (p = l.setBBoxZoom(p, ba.width() - 15, ba.height() - 15)) {
                        g = p.zoom;
                        p = p.bbox;
                        if (g < 0.001) sa({value: 0.1}); else {
                            a("#zoom").val(g * 100);
                            D ? qb() : qb(false, {x: p.x * g + p.width * g / 2, y: p.y * g + p.height * g / 2});
            ...
            var la = {}, ub = function (g) {
                a.each(g, function (p, D) {
                    var J = a(p).children(), M = p + "_show", Q = a(M), U = false;
                    ...
                    if (U) Q.attr("data-curopt", D[U].sel); else Q.attr("data-curopt") ||
                    Q.attr("data-curopt", D[0].sel);
                    var X, ga = a(M).position();
                    a(p).css({left: ga.left + 34, top: ga.top + 40});
                    Q.mousedown(function (ea) {
                        if (Q.hasClass("disabled"))return false;
                        var ka = a(p), ya = ga.left + 34, Da = ka.width() * -1, S = ka.data("shown_popop") ? 200 : 0;
                        X = setTimeout(function () {
                            Q.data("isLibrary") ? ka.css("left", ya).show() : ka.css("left", Da).show().animate({left: ya}, 150);
                        ...
                        if (Q.data("isLibrary") &&
                            a(M.replace("_show", "")).is(":visible")) La(M, true); else La(M) && ea in la && la[ea]()
                    })
                });
                Ib()
            }, Hb = function (g, p) {
                return a("<div>", {"class": "tools_flyout", id: g}).appendTo("#svg_editor").append(p)
            }, Ab = function () {
                a(".tools_flyout").each(function () {
                    var g = a("#" + this.id + "_show"), p = g.offset();
                    g = g.outerWidth();
                    a(this).css({left: (p.left + g) * lb, top: p.top})
                })
            }, Ib = function () {
                a(".tools_flyout").each(function () {
                ...
                if (g.indexOf("url(#") === 0) {
                    ...
                    J = {alpha: p};
                    J[g.tagName] = g
                } else J = g.indexOf("#") === 0 ? {alpha: p, solidColor: g.substr(1)} : {alpha: p, solidColor: "none"};
                ...
                var p = l.getCurrentDrawing().getCurrentLayerName(), D = l.getMode(),
                    J = f.baseUnit !== "px" ? f.baseUnit : null, M = D == "pathedit", Q = a("#cmenu_canvas li");
                ...
                if (g != null) {
                    var U = g.nodeName, X = l.getRotationAngle(g);
                    a("#angle").val(X);
                    var ga = l.getBlur(g);
                    a("#blur").val(ga);
                    ...
                    if (!M && D != "pathedit") {
                        ...
                        if (["line", "circle", "ellipse"].indexOf(U) >= 0) a("#xy_panel").hide(); else {
                            var ea, ka;
                    ...
                    if (J[ya]) {
                        ...
                        } else if (ya == "image") Bb(l.getHref(g)); else if (ya === "g" || ya === "use") {
            ...
            l.bind("selected", function (g, p) {
                ...
                Na();
                l.runExtensions("selectedChanged", {elems: p, selectedElement: ja, multiselected: Ca})
            });

            var selectedChanged = function(window, elems) {
                console.log("进入selectedChanged");
                var mode = svgCanvas.getMode();
                if (mode === "select")
                    setSelectMode();
                var is_node = (mode == "pathedit");
                // if elems[1] is present, then we have more than one
                // element
                selectedElement = (elems.length == 1 || elems[1] == null ? elems[0]
                    : null);
                multiselected = (elems.length >= 2 && elems[1] != null);
                if (selectedElement != null) {
                    // unless we're already in always set the mode of the
                    // editor to select because
                    // upon creation of a text element the editor is
                    // switched into
                    // select mode and this event fires - we need our UI to
                    // be in sync
            ...
            var togglePathEditMode = function(editmode, elems) {
                console.log("进入togglePathEditMode");
                $('#path_node_panel').toggle(editmode);
                var layerName = svgCanvas.getCurrentDrawing().getCurrentLayerName();
                console.log("layerName="+layerName);
                var layerType = svgCanvas.getLayerTypeByName(layerName);
                if(layerType == env.SVG_LAYER_TYPE.OUT_WALL){
                    console.log("layerType 1="+env.SVG_LAYER_TYPE.OUT_WALL);
                    $('#tools_bottom_2,#tools_bottom_3').toggle(false);
                }else{
                    console.log("layerType 2="+env.SVG_LAYER_TYPE.OUT_WALL);
                    $('#tools_bottom_2,#tools_bottom_3').toggle(!editmode);
                }
                if (editmode) {
                    // Change select icon
                    console.log("editmode 1="+editmode);
                    $('.tool_button_current').removeClass(
            ...
            l.bind("changed", function (g, p) {
                ...
                l.runExtensions("elementChanged", {elems: p})
            });
            l.bind("saved", function (g, p) {
                ...
                if (~D.indexOf("Chrome") && a.browser.version >= 533 || ~D.indexOf("MSIE")) m(0,
                    true); else {
                    var J = g.open("data:image/svg+xml;base64," + Ya.encode64(p)), M = a.pref("save_notice_done");
                    if (M !== "all") {
            ...
            l.bind("extension_added", function (g, p) {
                ...
                var J = false, M = false, Q = true, U = function () {
                    if (p.callback && !J && Q) {
                        J = true;
                        p.callback()
                    }
                ...
                p.context_tools && a.each(p.context_tools, function (Da, S) {
                    var Aa = S.container_id ? ' id="' + S.container_id + '"' : "", fa = a("#" + S.panel);
                    fa.length || (fa = a("<div>", {id: S.panel}).appendTo("#tools_top"));
                    switch (S.type) {
                        case "tool_button":
                            var ta = '<div class="tool_button">' + S.id + "</div>", Ea = a(ta).appendTo(fa);
                        ...
                        case "button-select":
                            ...
                            X.push({
                                elem: "#" + S.id, list: "#" + S.id + "_opts", title: S.title,
                                callback: S.events.change, cur: "#cur_" + S.id
                            });
                            break;
                        case "input":
                ...
                if (p.buttons) {
                    ...
                    a.each(p.buttons, function (Da, S) {
                        ...
                        if (ka) {
                            ...
                            if (S.type == "app_menu") ea["#" + fa + " > div"] = ta; else ea["#" + fa] = ta
                        ...
                        switch (S.type) {
                            ...
                            case "context":
                                Ea = "tool_button";
                                Ka = "#" + S.panel;
                                a(Ka).length || a("<div>", {id: S.panel}).appendTo("#tools_top");
                                break;
                            case "app_menu":
                                Ea = "";
                                Ka = "#main_menu ul"
                        ...
                        if (!S.includeWith && !S.list) {
                            ...
                            if (S.type == "mode_flyout") {
                                ...
                                if (!ta.parent().hasClass("tools_flyout")) {
                                    var Ba = ta[0].id.replace("tool_", "tools_"),
                                        ab = ta.clone().attr("id", Ba + "_show").append(a("<div>", {"class": "flyout_arrow_horiz"}));
                                    ta.before(ab);
                                    Ea = Hb(Ba, ta);
                            ...
                            if (!isNaN(Ba) && Ba >= 0 && Ba < hc) Ea.children().eq(Ba).before(Ma); else {
                                Ea.append(Ma);
                                fa.reverse()
                            }
                        }
                        ...
                        S.list || a.each(S.events, function (ac, bc) {
                            ...
                            } else Ma.bind(ac, bc); else Ma.bind(ac, bc)
                        });
                        ub(ya)
                    });
                    a.each(X, function () {
                        wa(this.elem, this.list, this.callback, {seticon: true})
                    });
                    if (ka) Q = false;
                    a.svgIcons(ka, {
                        w: 24,
            ...
            if (a.pref("bkgd_color")) A(a.pref("bkgd_color"), a.pref("bkgd_url")); else a.pref("bkgd_url") && A(G.bkgd_color, a.pref("bkgd_url"));
            if (a.pref("img_save")) {
            ...
            a("#selLayerNames").change(function () {
                var g = this.options[this.selectedIndex].value, p = k.notification.QmoveElemsToLayer.replace("%s",
                    g), D = function (J) {
                    if (J) {
                        Sa = true;
            ...
            a(".attr_changer").change(function () {
                var g = this.getAttribute("data-attr"), p = this.value;
                if (!svgedit.units.isValidUnit(g, p, ja)) {
                    a.alert(k.notification.invalidAttrValGiven);
                    this.value = ja.getAttribute(g);
                    return false
                }
                if (g !== "id")if (isNaN(p)) p = l.convertToNum(g, p); else if (f.baseUnit !== "px") {
            ...
            a(".palette_item").mousedown(function (g) {
                ...
                var D = a(this).attr("data-rgb"), J = null;
                if (D === "none" || D === "transparent" || D === "initial") {
                    D = "none";
                    J = new a.jGraduate.Paint
                } else J = new a.jGraduate.Paint({alpha: 100, solidColor: D.substr(1)});
                Oa[p].setPaint(J);
                if (g) {
                    l.setColor("stroke", D);
            ...
            (function () {
                var g = null, p = null, D = ba[0], J = false, M = false;
            ...
            (function () {
                var g = a("#main_icon"), p = a("#main_icon span"), D = a("#main_menu"), J = false, M = 0, Q = true,
                    U = false;
                a(window).mouseup(function (ga) {
                    if (!J) {
                        g.removeClass("buttondown");
                        if (ga.target.tagName != "INPUT") D.fadeOut(200); else if (!U) {
                            U = true;
                ...
                p.bind("mousedown", function () {
                    if (g.hasClass("buttondown")) {
                        ...
                        M || (M = D.height());
                        D.css("height", 0).animate({height: M}, 200);
                        J = true;
                        return false
                    }
                }).hover(function () {
            ...
            s.addDropDown = function (g, p, D) {
                if (a(g).length != 0) {
                    var J = a(g).find("button"), M = a(g).find("ul").attr("id", a(g)[0].id + "-list");
                    ...
                    J.bind("mousedown", function () {
                        if (J.hasClass("down")) {
                            ...
                            if (!D) {
                                var U = a(g).position();
                                M.css({top: U.top + 24, left: U.left - 10})
                            }
                            M.show();
                            Q = true
                        }
            ...
            s.addDropDown("#zoom_dropdown", function () {
                var g = a(this), p = g.attr("data-val");
                p ? ra(window, p) : sa({value: parseInt(g.text())})
            }, true);
            ...
            var Cb = function () {
                ...
                Vb = function () {
                    ...
                    var g = l.getZoom(), p = (ba[0].scrollLeft + ba.width() / 2) / g - l.contentW;
                    ...
                    var g = a("#tool_add_subpath"), p = !g.hasClass("push_button_pressed");
            ...
            var i = function () {
                    ...
                    if (!Ia) {
                        Ia =
                            true;
                        a("#main_menu").hide();
                        var g = a("#bg_blocks div"), p = a.pref("bkgd_color"), D = a.pref("bkgd_url");
                        g.each(function () {
                            var J = a(this), M = J.css("background-color") == p;
                ...
                o = function () {
                    ...
                    g = a("#canvas_width");
                    var p = g.val(), D = a("#canvas_height"), J = D.val();
            ...
            C = function () {
                var g = /^(Moz|Webkit|Khtml|O|ms|Icab)(?=[A-Z])/, p = document.getElementsByTagName("script")[0], D;
            ...
            var E = function (g, p) {
                ...
                g.each(function () {
                    for (var J = a(this), M = J.outerWidth() *
                        ...
                        var X = D[U], ga = J.data("orig_margin-" + X);
                        if (ga == null) {
                            ga = parseInt(J.css("margin-" + X));
                            J.data("orig_margin-" + X, ga)
                        }
                        ga = ga * p;
                        if (X === "right") ga += M; else if (X === "bottom") ga += Q;
                        J.css("margin-" + X, ga)
                    }
                })
                ...
                if (!(g == e.size && !p)) {
                    ...
                    var D = a("#tools_top .toolset, #editor_panel > *, #history_panel > *,\t\t\t\t#main_button, #tools_left > *, #path_node_panel > *, #multiselected_panel > *,\t\t\t\t#g_panel > *, #tool_font_size > *, .tools_flyout"),
                        J = 1;
                    J = typeof g == "number" ? g : {s: 0.75, m: 1, l: 1.25, xl: 1.5}[g];
                    s.tool_scale = lb = J;
                    Ab();
                    ...
                    a("#iconsize").val(g);
                    M = {
                        "#tools_top": {left: 50, height: 72},
                        "#tools_left": {width: 31, top: 74},
                        "div#workarea": {left: 38, top: 74}
                    };
                    D = a("#tool_size_rules");
                    if (D.length) D.empty(); else D = a('<style id="tool_size_rules"></style>').appendTo("head");
                    if (g != "m") {
                        var Q = "";
                        a.each(M, function (U, X) {
                            ...
                            a.each(X, function (ga, ea) {
                                if (typeof ea === "number")var ka = ea * J + "px"; else if (ea[g] || ea.all) ka = ea[g] || ea.all;
                                Q += ga + ":" + ka + ";"
                            });
                            Q += "}"
                        });
                ...
                a("#svg_prefs").hide();
                Ia = false
            }, R = {width: a(window).width(), height: a(window).height()}, T = a.noop, V;
            svgedit.browser.isIE() && function () {
                T = function () {
                ...
                V = {left: ba[0].scrollLeft, top: ba[0].scrollTop};
                a(window).resize(T);
            ...
            if (ca && !window.opera) {
                ...
                for (var aa = ca.length; aa--;) {
                    ...
                    if (Ja != null) {
                        var Za = Ja.title, mb = Za.indexOf("Ctrl+");
                        Ja.title = [Za.substr(0, mb), "Cmd+", Za.substr(mb + 5)].join("")
                    }
                }
            }
            var eb = function (g) {
                var p = g.attr("id") == "stroke_color" ? "stroke" : "fill", D = Oa[p].paint,
                ...
                a("#color_picker").draggable({
                    ...
                    {left: g.left - 140, bottom: 40}).jGraduate({
                    paint: D,
                    window: {pickerTitle: J},
                    images: {clientPath: f.jGraduatePath},
                    newstop: "inverse"
                }, function (M) {
                ...
                var g = l.getColor("fill") == "none", p = l.getColor("stroke") == "none",
                ...
                l.runExtensions("toolButtonStateUpdate", {nofill: g, nostroke: p});
                a(".tools_flyout").each(function () {
                    var U = a("#" + this.id + "_show"), X = false;
                    a(this).children().each(function () {
            ...
            ca = function (g, p) {
                ...
                this.setPaint = function (M, Q) {
                    this.paint = M;
                    var U = "none", X = M.type, ga = M.alpha / 100;
                    switch (X) {
                        case "solidColor":
                            U = M[X] != "none" ? "#" + M[X] : M[X];
                            break;
                ...
                this.update = function (M) {
                    if (ja) {
                        ...
                        switch (ja.tagName) {
                            ...
                            case "g":
                            case "a":
                                for (var U = null, X = ja.getElementsByTagName("*"), ga = 0,
                                         ea = X.length; ga < ea; ga++) {
                                    var ka = X[ga].getAttribute(Q);
                                    if (ga === 0) U = ka; else if (U !== ka) {
                                        U = null;
                                        break
                                    }
                                }
                ...
                this.prep =
                    function () {
                        switch (this.paint.type) {
                            case "linearGradient":
                            case "radialGradient":
                                var M = new a.jGraduate.Paint({copy: this.paint});
                                l.setPaint(p, M)
                        }
                    }
            };
            ...
            a(ca).remove();
            (function () {
                var g = "-" + C.toLowerCase() + "-zoom-", p = g + "in";
                ba.css("cursor", p);
                if (ba.css("cursor") === p) {
                    Tb = p;
                    bb = g + "out"
            ...
            a("#layer_new").click(function () {
                ...
                /*a.prompt(k.notification.enterUniqueLayerName,
                    p, function (D) {
                        if (D)if (l.getCurrentDrawing().hasLayer(D)) a.alert(k.notification.dupeLayerName); else {
                            l.createLayer(D);
                            Na();
                            $a()
                        }
                ...
                var i = svgCanvas.getCurrentDrawing()
                    .getNumLayers();
                do {
                    var uniqName = k.layers.layer
                        + " " + ++i;
                } while (svgCanvas.getCurrentDrawing()
                    .hasLayer(uniqName));

                a.newlayer(
                        k.notification.enterUniqueLayerName,
                        {
                            layername : uniqName,
                            layertype : ""
                        },
                        function(jsonobj) {
                            ...
                            if (!jsonobj.layername){
                                $.alert("图层名不能为空！");
                                return;
                            }
                            if(!jsonobj.layertype){
                                $.alert("图层类型不能为空！");
                                return;
                            }
                            if (svgCanvas
                                    .getCurrentDrawing()
                                    .hasLayer(jsonobj.layername)) {
                                $.alert(uiStrings.notification.dupeLayerName);
                                return;
                            }
                            //判断图层类型是否存在
                            var layerTypeList = svgCanvas.getLayerTypeList();
                            if(layerTypeList[jsonobj.layertype]){
                                $.alert("图层类型已存在！");
                                return;
                            }
                            //svgCanvas.tool_selectClick();
                            //var a = $('.layertype')
                            //		.html();
                            svgCanvas.createLayer(jsonobj.layername,jsonobj.layertype);
                            updateContextPanel();
                            populateLayers();
                        });
            });
            ...
            var updateContextPanel = function() {
                ...
                var unit = curConfig.baseUnit !== 'px' ? curConfig.baseUnit
                    : null;

                var is_node = currentMode == 'pathedit'; // elem ?
                // (elem.id &&
            ...
            #ellipse_panel, #line_panel, #text_panel, #image_panel, #container_panel, #use_panel, #a_panel')
                    .hide();
                if (elem != null) {
                    var elname = elem.nodeName;

                    // If this is a link with no transform and one child,
                    // pretend
                    ...
                    if (svgCanvas.addedNew) {
                         ...
                         * else if(elname == 'text') { // TODO: Do
                         * something here for new text }
                         */
                    }
                    ...
                    if (!is_node && currentMode != 'pathedit') {
                        ...
                        // Elements in this array cannot be converted to a
                        // path
                        var no_path = [ 'image', 'text', 'path', 'g', 'use' ]
                                .indexOf(elname) == -1;
                        $('#tool_topath').toggle(no_path);
                        $('#tool_reorient').toggle(elname == 'path');
                        $('#tool_reorient').toggleClass('disabled',
                            angle == 0);
                    } else {
                        var point = path.getNodePoint();
                        $('#tool_add_subpath').removeClass(
                            'push_button_pressed').addClass(
                            'tool_button');
                        $('#tool_node_delete').toggleClass('disabled',
                            !path.canDeleteNodes);

                        // Show open/close button based on selected point
                        setIcon('#tool_openclose_path',
                            path.closed_subpath ? 'open_path'
                                : 'close_path');

                        if (point) {
                            var seg_type = $('#seg_type');
                            if (unit) {
                                point.x = svgedit.units
                                    .convertUnit(point.x);
                                point.y = svgedit.units
                                    .convertUnit(point.y);
                            }
                            $('#path_node_x').val(point.x);
                            $('#path_node_y').val(point.y);
                            if (point.type) {
                                seg_type.val(point.type).removeAttr(
                                    'disabled');
                            } else {
                                seg_type.val(4)
                    ...
                    if (panels[el_name]) {
                        ...
                        if (el_name == 'text') {
                            ...
                            if (svgCanvas.getItalic()) {
                                $('#tool_italic').addClass(
                                    'push_button_pressed').removeClass(
                                    'tool_button');
                            } else {
                                $('#tool_italic').removeClass(
                                    'push_button_pressed').addClass(
                                    'tool_button');
                            }
                    ...
                    menu_items[((el_name === 'g' || !multiselected) ? 'dis'
                        : 'en')
                    + 'ableContextMenuItems']('#group');
                } // if (elem != null)
                    ...
                    menu_items
                        .disableContextMenuItems('#modifyPoiInfo,#delete,#cut,#copy,#group,#ungroup,#move_front,#move_up,#move_down,#move_back');
                }
                ...
                if ((elem && !is_node) || multiselected) {
                    ...
                    $('#selLayerNames').removeAttr('disabled').val(
                        currentLayerName);

                    // Enable regular menu options
                    canv_menu
                        .enableContextMenuItems('#delete,#cut,#copy,#move_front,#move_up,#move_down,#move_back');
                    var layerName = svgCanvas.getCurrentDrawing()
                        .getCurrentLayerName();
                    var layerType = svgCanvas.getLayerTypeByName(layerName);
                    if (((layerType == env.SVG_LAYER_TYPE.POI) || (layerType == env.SVG_LAYER_TYPE.AP))
                        && (!multiselected)) {
                        menu_items.enableContextMenuItems("#modifyPoiInfo");
                        if(layerType == env.SVG_LAYER_TYPE.AP){
                            $("#ID_modifyPoiInfo").html("修改AP信息");
                        }else{
                            $("#ID_modifyPoiInfo").html("修改POI信息");
                        }
                    } else {
                        menu_items
                            .disableContextMenuItems("#modifyPoiInfo");
                    }
                } else {
            ...
            var toggleHighlightLayer = function(layerNameToHighlight) {
                var curNames = new Array(svgCanvas.getCurrentDrawing()
                    .getNumLayers());
                for ( var i = 0; i < curNames.length; ++i) {
                    curNames[i] = svgCanvas.getCurrentDrawing()
                        .getLayerName(i);
                }
                ...
                if (layerNameToHighlight) {
                    for ( var i = 0; i < curNames.length; ++i) {
                        if (curNames[i] != layerNameToHighlight) {
                            svgCanvas.getCurrentDrawing().setLayerOpacity(
                                curNames[i], 0.5);
                        }
                    }
                } else {
            ...
            var populateLayers = function() {
                ...
                var currentLayerName = svgCanvas.getCurrentDrawing()
                    .getCurrentLayerName();
                svgCanvas.setLayerShape(currentLayerName);
                setTimeout(function() {
                    svgCanvas.setLayerShape(currentLayerName);
                }, 1200);
                var layernumbers = svgCanvas.getCurrentDrawing()
                    .getNumLayers();
                var layer = layernumbers;
                // we get the layers in the reverse z-order (the layer
                // rendered on top is listed first)
                while (layer--) {
                    var name = svgCanvas.getCurrentDrawing().getLayerName(
                        layer);
                    ...
                    appendstr += "\">";
                    var type = svgCanvas.getLayerTypeByNum(layer);
                    var typehtml = env.typeJson[type];
                    if (typehtml == undefined) {
                        typehtml = "";
                    }
                    var layerinvis = svgCanvas.getCurrentDrawing()
                        .getLayerVisibility(name) ? "" : " layerinvis";
                    var layerdeletetd = (layernumbers == 1) ? "<td style=\"width:14px\"></td>"
                        : "<td class=\"layerdelete\" style=\"cursor:pointer;\"></td>";
                    var layerrenametd = "<td class=\"layerrename\" style=\"cursor:pointer;\"></td>";
                    appendstr += "<td class=\"layername\" >"
                        + name
                        + "</td>"
                        + "<td class=\"layertype\" style=\"width:50px\">"
                        + typehtml + "</td><td class=\"layervis"
                        + layerinvis + "\"/>" + layerrenametd
                        + layerdeletetd + "</tr>";
                    layerlist.append(appendstr);
                    selLayerNames.append("<option value=\"" + name + "\">"
                        + name + "</option>");
                }
                ...
                var icon = $.getSvgIcon('text');
                if (icon !== undefined) {
                    $('td.layerrename', layerlist).append(icon.clone());
                    $.resizeSvgIcons({
                        'td.layerrename .svg_icon' : 14
                    });
                }
                var icon = $.getSvgIcon('delete');
                if (icon !== undefined) {
                    $('td.layerdelete', layerlist).append(icon.clone());
                    $.resizeSvgIcons({
                        'td.layerdelete .svg_icon' : 14
                    });
                }
                ...
                $('#layerlist td.layername').mouseup(function(evt) {
                    ...
                    $(this).css(style_mouseover);
                    toggleHighlightLayer(this.textContent);
                }).mouseout(function(evt) {
                    $(this).css(style_mouseout);
                    toggleHighlightLayer();
                });
                $('#layerlist td.layertype').mouseup(
                    function(evt) {
                        //svgCanvas.tool_selectClick();
                        $('#layerlist tr.layer')
                        ...
                        var row = $(this.parentNode);
                        row.addClass("layersel");
                        svgCanvas.setCurrentLayer(row.children(
                            "td.layername").text());
                        evt.preventDefault();
                    }).mouseover(
                    function(evt) {
                        var layername = $(this.parentNode).children(
                            "td.layername");
                        layername.css(style_mouseover);
                        console.log("function(evt)="+layername);
                        toggleHighlightLayer(layername.text());
                    }).mouseout(
                    function(evt) {
                        var layername = $(this.parentNode).children(
                            "td.layername");
                        layername.css(style_mouseout);
                        console.log("function(evt)="+layername);
                        toggleHighlightLayer();
                    });
                $('#layerlist td.layerdelete').mouseup(
                    function(evt) {
                        $('#layerlist tr.layer')
                            .removeClass("layersel");
                        var row = $(this.parentNode);
                        row.addClass("layersel");
                        svgCanvas.setCurrentLayer(row.children("td.layername").text());
                        //var layername=row.children(
                        //"td.layername").text();
                        //var LayerType=svgCanvas.getLayerTypeByName(layername);
                ...
                $('#layerlist td.layerrename')
                    .click(
                        function(evt) {
                            var row = $(this.parentNode);
                            var oldName = row.children(
                                "td.layername").text();
                            var oldType = svgCanvas
                                .getLayerTypeByName(oldName);
                            $
                                .newlayer(
                                    ...
                                    function(jsonobj) {
                                        ...
                                        if ((oldName != jsonobj.layername)
                                            || (oldType != jsonobj.layertype)) {
                                            svgCanvas
                                                .renametypeLayerByName(
                                                    oldName,
                                                    jsonobj.layername,
                                                    jsonobj.layertype);
                                            updateContextPanel();
                            ...
                            // 图层类型为AP或POI时，修改图层不能修改类型
                            if (oldType == env.SVG_LAYER_TYPE.AP || oldType == env.SVG_LAYER_TYPE.POI) {
                                $('#changeLayerType').attr(
                                    "disabled", "true");
                            } else {
                                $('#changeLayerType').removeAttr(
                                    "disabled");
                            }
                        });
                $('#layerlist td.layerdelete').click(function(evt) {
                    deleteLayer();
                });
            ...
            function deleteLayer() {
                if (svgCanvas.deleteCurrentLayer()) {
                    ...
                    // TODO: make this behavior less brittle (svg-editor
                    // should get which
                    // layer is selected from the canvas and then select
                    // that one in the UI)
                    $('#layerlist tr.layer').removeClass("layersel");
            ...
            var wb = -1, ic = false, nc = false, pc = function (g) {
                if (nc)if (wb != -1) {
                    ic = true;
                    g = wb - g.pageX;
                    var p = a("#sidepanels"), D = parseInt(p.css("width"));
                    if (D + g > 300) g = 300 - D; else if (D + g < 2) g = 2 - D;
                    if (g != 0) {
                        wb -= g;
                        D = a("#layerpanel");
            ...
            var oc = function (g) {
                ...
                g = (p > 2 || g ? 2 : 150) - p;
                p = a("#sidepanels");
                var D = a("#layerpanel"), J = a("#ruler_x");
                ba.css("right", parseInt(ba.css("right")) + g);
                p.css("width", parseInt(p.css("width")) + g);
                D.css("width", parseInt(D.css("width")) + g);
                ...
                for (var p =
                        Array(l.getCurrentDrawing().getNumLayers()),
                         D = 0; D < p.length; ++D)p[D] = l.getCurrentDrawing().getLayerName(D);
                if (g)for (D = 0; D < p.length; ++D)p[D] != g && l.getCurrentDrawing().setLayerOpacity(p[D], 0.5); else for (D = 0; D < p.length; ++D)l.getCurrentDrawing().setLayerOpacity(p[D], 1)
            }, $a = function () {
                var g = a("#layerlist tbody"), p = a("#selLayerNames");
                g.empty();
                p.empty();
                for (var D = l.getCurrentDrawing().getCurrentLayerName(), J = l.getCurrentDrawing().getNumLayers(),
                         M = a.getSvgIcon("eye"); J--;) {
                ...
                a("#layerlist td.layername").mouseup(function (X) {
                    ...
                    a(this).css({"font-style": "italic", color: "blue"});
                    qc(this.textContent)
                }).mouseout(function () {
                    a(this).css({"font-style": "normal", color: "black"});
                    qc()
                });
                /*自定义 */
                $('#layerlist td.layertype').mouseup(
            ...
            a("#resolution").change(function () {
                ...
                if (this.selectedIndex)if (this.value == "content") g.val("fit").attr("disabled", "disabled"); else {
            ...
            var mc = function () {
                var g = [{sel: "#tool_select", fn: Cb, evt: "click", key: ["V", true]}, {
                        sel: "#tool_fhpath",
                        fn: Vb,
                        evt: "click",
                        key: ["Q", true]
                    }, {sel: "#tool_line", fn: Ub, evt: "click", key: ["L", true]}, {
                        sel: "#tool_rect",
                        fn: Eb,
                        evt: "mouseup",
                        key: ["R", true],
                        ...
                        parent: "#tools_rect",
                        icon: "square"
                    }, {sel: "#tool_fhrect", fn: Wb, evt: "mouseup", parent: "#tools_rect", icon: "fh_rect"},
                        {
                            sel: "#tool_ellipse",
                            fn: jc,
                            evt: "mouseup",
                            ...
                            parent: "#tools_ellipse",
                            icon: "fh_ellipse"
                        }, {sel: "#tool_path", fn: rb, evt: "click", key: ["P", true]}, {
                            sel: "#tool_text",
                            fn: ha,
                            evt: "click",
                            key: ["T", true]
                        }, {sel: "#tool_image", fn: vb, evt: "mouseup"}, {
                            sel: "#tool_zoom",
                            fn: db,
                            evt: "mouseup",
                            key: ["Z", true]
                        }, {
                            sel: "#tool_clear", fn: gc, evt: "mouseup", key: ["N",
                                true]
                        }, {
                            sel: "#tool_save", fn: function () {saveSvg()
                                /*Ha ? h() : l.save({images: e.img_save, round_digits: 6})*/
                            }, evt: "mouseup", key: ["S", true]
                        }, {sel: "#tool_export", fn: lc, evt: "mouseup"}, {
                            sel: "#tool_open",
                            fn: xb,
                            evt: "mouseup",
                            key: ["O", true]
                        }, {sel: "#tool_import", fn: Xa, evt: "mouseup"}, {
                            sel: "#tool_source",
                            fn: m,
                            evt: "click",
                            key: ["U", true]
                        ...
                        {sel: "#tool_source_save", fn: h, evt: "click"}, {
                            sel: "#tool_docprops_save",
                            fn: w,
                            evt: "click"
                        },
                        {
                            sel : '#SVG_VIEWBOX_WIDTH',
                            fn : setViewboxSize,
                            evt : 'click'
                        },
                        {
                            sel : '#SVG_VIEWBOX_BACKGROUNDCOLOR',
                            fn : setViewBoxBackground,
                            evt : 'click'
                        },
                        {
                            sel : '#SVG_DW_UNIT',
                        ...
                        {sel: "#tool_docprops", fn: i, evt: "mouseup"}, {
                            sel: "#tool_prefs_save",
                            fn: u,
                            evt: "click"
                        }, {
                        ...
                        }, {sel: "#tool_reorient", fn: za, evt: "click"}, {
                            sel: "#tool_node_link",
                            fn: Pb,
                            evt: "click"
                        }, {sel: "#tool_node_clone", fn: Mb, evt: "click"}, {
                            ...
                            fn: fc, evt: "click"
                        }, {sel: "#tool_add_subpath", fn: qa, evt: "click"}, {
                            sel: "#tool_move_top",
                            fn: Fb,
                            evt: "click",
                            key: "ctrl+shift+]"
                        }, {sel: "#tool_move_bottom", fn: dc, evt: "click", key: "ctrl+shift+["}, {
                            sel: "#tool_topath",
                            fn: nb,
                            evt: "click"
                        }, {sel: "#tool_make_link,#tool_make_link_multi", fn: gb, evt: "click"}, {
                            sel: "#tool_undo",
                            fn: Zb,
                            evt: "click",
                            key: ["Z", true]
                            ...
                            evt: "click",
                            key: ["Y", true]
                        }, {sel: "#tool_clone,#tool_clone_multi", fn: Va, evt: "click", key: ["D", true]}, {
                            sel: "#tool_group", fn: ob, evt: "click",
                            key: ["G", true]
                        }, {sel: "#tool_ungroup", fn: ob, evt: "click"}, {
                            sel: "#tool_unlink_use",
                            fn: ob,
                            evt: "click"
                        }, {sel: "[id^=tool_align]", fn: Yb, evt: "click"}, {
                            sel: "#tool_bold",
                            fn: zb,
                            evt: "mousedown"
                        }, {sel: "#tool_italic", fn: Qb, evt: "mousedown"}, {
                            sel: "#sidepanel_handle",
                            fn: oc,
                            key: ["X"]
                        }, {sel: "#copy_save_done", fn: I, evt: "click"}, {
                            ...
                            key: [pa + "up", true], fn: function () {
                                b(2)
                            }
                        }, {
                            key: [pa + "down", true], fn: function () {
                                b(0.5)
                            }
                        }, {
                            key: [pa + "]", true], fn: function () {
                                Ta("Up")
                            }
                        }, {
                            key: [pa + "[", true], fn: function () {
                        ...
                        }, {key: pa + "z", fn: Zb}, {key: pa + "shift+z", fn: Sb}, {key: pa + "y", fn: Sb}, {
                            key: pa + "x",
                            fn: Ob
                        }, {key: pa + "c", fn: Xb}, {key: pa + "v", fn: kc}],
                    p = {"4/Shift+4": "#tools_rect_show", "5/Shift+5": "#tools_ellipse_show"};
                return {
                    setAll: function () {
                        var D = {};
                        ...
                        a.each(p, function (D, J) {
                            ...
                            a(J).each(function () {
                                ...
                                if (M) this.lastChild.textContent = Q + " [" + U + "]"; else this.title = Q + " [" + U + "]"
                            })
                        })
            ...
            mc.setAll();
            s.ready(function () {
                var g = f.initTool, p = a("#tools_left, #svg_editor .tools_flyout"), D = p.find("#tool_" + g);
                g = p.find("#" + g);
            ...
            a("#rect_rx").SpinButton({
                min: 0, max: 1E3, step: 1, callback: function (g) {
                    l.setRectRadius(g.value)
                }
            });
            a("#stroke_width").SpinButton({
                min: 0, max: 99, step: 1, smallStep: 0.1, callback: function (g) {
                    var p = g.value;
            ...
            a("#angle").SpinButton({
                min: -180, max: 180, step: 5, callback: function (g) {
                    l.setRotationAngle(g.value);
                    a("#tool_reorient").toggleClass("disabled", g.value == 0)
                }
            });
            a("#font_size").SpinButton({
                step: 1, min: 0.001, stepfunc: function (g, p) {
                    var D = g.value - 0, J = D + p, M = J >= D;
                    if (p === 0)return D;
            ...
            a("#group_opacity").SpinButton({
                step: 5,
                min: 0, max: 100, callback: cb
            });
            a("#blur").SpinButton({step: 0.1, min: 0, max: 10, callback: pb});
            a("#zoom").SpinButton({
                min: 0.001, max: 1E4, step: 50, stepfunc: function (g, p) {
                    var D = g.value - 0;
                    if (D === 0)return 100;
                    var J = D + p;
                    if (p === 0)return D;
            ...
            a("#workarea").contextMenu({menu: "cmenu_canvas", inSpeed: 0}, function (g) {
                switch (g) {
                    case "delete":
                        Nb();
                        break;
            ...
            a("#layerlist").contextMenu({menu: "cmenu_layers", inSpeed: 0}, ca);
            a("#layer_moreopts").contextMenu({menu: "cmenu_layers", inSpeed: 0, allowLeft: true}, ca);
            ...
            if (window.FileReader) {
                ca = function (g) {
                    ...
                    if (p = g.type == "drop" ? g.dataTransfer.files[0] :
                            this.files[0])if (p.type.indexOf("image") != -1)if (p.type.indexOf("svg") != -1) {
                        g = new FileReader;
            ...
            var qb = s.updateCanvas = function (g, p) {
                var D = ba.width(), J = ba.height(), M = D, Q = J, U =
                ...
                var ka = X.width() / 2;
                X.width(D).height(J);
                var ya = J / 2, Da = D / 2, S = l.updateCanvas(D, J), Aa = Da / ka;
                D = D / 2 - M / 2;
                J = J / 2 - Q / 2;
                if (p) {
                    p.x += S.x;
                    p.y += S.y
                } else p = {x: Da + (ga.x - ka) * Aa, y: ya + (ga.y - ea) * Aa};
            ...
            try {
                var rc = function (g) {
                    ...
                    if (typeof g == "boolean" || typeof g == "number")return g + ""; else if (typeof g == "string")return '"' + g.replace(/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
                            function (M) {
        ...
        s.loadFromURL = function (B, A) {
            A || (A = {});
            var O = A.cache, Z = A.callback;
            s.ready(function () {
                a.ajax({
                    url: B, dataType: "text",
                    cache: !!O, success: function (N) {
                        K(N, Z)
...
(function () {
    ...
    svgedit.contextmenu.add = function (K) {
        if (K && K.id && K.label && K.action && typeof K.action == "function")if (K.id in a.contextMenuExtensions) console.error('Cannot add extension "' + K.id + '", an extension by that name already exists"'); else {
...
var svgEditor = function (a, K) {
    ...
    K.readLang = function (v) {
        ...
        if (v.tools) {
            var e = v.tools;
            G = v.properties;
            var f = v.config, k = v.layers, n = v.common, F = v.ui;
            l("content", {
    ...
    K.putLocale = function (v, G) {
        if (v) s = v; else {
            s = a.pref("lang");
            if (!s) {
                if (navigator.userLanguage) s = navigator.userLanguage; else if (navigator.language) s = navigator.language;
                if (s == "")return
            }
            console.log("Lang: " + s);
...
var setViewboxSize = function() {
    var w = $('#SVG_VIEWBOX_WIDTH').val();
    var h = $('#SVG_VIEWBOX_HEIGHT').val();
...
var setViewBoxBackground = function() {
    var color = $('#SVG_VIEWBOX_BACKGROUNDCOLOR').val();
    setBackground(color, "");
}
var saveConfigUnit = function() {
    curConfig.baseUnit = $('#SVG_DW_UNIT').val();
