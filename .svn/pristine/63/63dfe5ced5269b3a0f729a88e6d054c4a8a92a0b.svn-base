...
svgedit.draw.Drawing = function(svgElem, opt_idPrefix) {
	 ...
	 * @type {number}
	 */
	this.obj_num = new UUID().toString();
	
	/**
	 * The prefix to prepend to each element id in the drawing.
	 * @type {String}
	 */
	//this.idPrefix = opt_idPrefix || curConfig.idPrefix;
	if(opt_idPrefix != null) {
		this.idPrefix = opt_idPrefix;
	} else {
		this.idPrefix = curConfig.idPrefix;
	}
...
svgedit.draw.Drawing.prototype.getId = function() {
	...
	return (this.idPrefix + (this.nonce_ ? (this.nonce_ +'_') : "") + this.obj_num);
};
...
svgedit.draw.Drawing.prototype.releaseId = function(id) {
	...
	// then return false.
	if (isNaN(num) || typeof num != typeof 1 || num <= 0 || this.releasedNums.indexOf(num) != -1) {
		return false;
	}
...
// of this drawing.
svgedit.draw.Drawing.prototype.createLayer = function(name,type) {
	var svgdoc = this.svgElem_.ownerDocument;
	var new_layer = svgdoc.createElementNS(svg_ns, "g");
	var layer_title = svgdoc.createElementNS(svg_ns, "title");
	var layer_type = svgdoc.createElementNS(svg_ns, "desc");
...
svgedit.draw.Drawing.prototype.createElement = function(name) {
	var svgdoc = this.svgElem_.ownerDocument;
	var tag = svgdoc.createElementNS(svg_ns, name);
	return (tag);
};
svgedit.draw.Drawing.prototype.getLayerByName = function(layername) {
	for ( var i = 0; i < this.getNumLayers(); ++i) {
		if (this.getLayerName(i) == layername) {
			return (this.all_layers[i][1]);
		}
	}
	return (null);
};
