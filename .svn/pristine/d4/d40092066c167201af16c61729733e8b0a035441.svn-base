...
// Default configuration options
    ...
    if(viewboxPosFix){
        curConfig.dimensions = [viewboxPosX,viewboxPosY];
    }
...
// Prefix string for element IDs
    var idprefix = curConfig.idprefix;
...
// An array with all "visible" elements.
    var getVisibleElements = this.getVisibleElements = function(parent) {
        ...
        $(parent).children().each(function(i, elem) {
            try {
                if (!!elem && !!elem.getBBox && elem.getBBox()) {
...
// Mouse events
    (function() {
        ...
        var mouseDown = function(evt)
        {
            ...
            var pt = transformPointForFireFox( evt.clientX, evt.clientY, root_sctm ),
...
// 					}
        ...
        function transformPointForFireFox(x, y, m){
            var pt = transformPoint(x, y, m);
            if (isFirefox) {
                pt.x = (pt.x - svgcontent.x.animVal.value) / current_zoom;
                pt.y = (pt.y - svgcontent.y.animVal.value) / current_zoom;
            }
            return (pt);
        }
        ...
        {
            var need_finish_resize = false;
            var right_click = evt.button === 2;
            if(right_click) {
                if(current_mode == "resize") {
                    need_finish_resize = true;
                }else{
                    return;
                }
            }
...
// 							call("selected", [selected]);
                        ...
                        if ((real_x != r_start_x || real_y != r_start_y) || need_finish_resize) {
                            var len = selectedElements.length;
                            for	(var i = 0; i < len; ++i) {
            ...
            if (!keep && element != null) {
                ...
                while ((t.parentNode != null) && (t.parentNode.parentNode.tagName == "g")) {
                    t = t.parentNode;
                }
                ...
                if ( (current_mode != "path" || !drawn_path) &&
                    ((t.parentNode != null) && (t.parentNode.id != "selectorParentGroup")) &&
                ...
                var stroke_width = cur_shape.stroke_width;
                var fill = cur_shape.fill;
                var layerName = svgCanvas.getCurrentDrawing().getCurrentLayerName();
                var layerType = svgCanvas.getLayerTypeByName(layerName);
                switch(layerType){
                    case env.SVG_LAYER_TYPE.POI:
                        if(current_mode != "image") {
                            ShowPoiInfo(getId(), layerType, false);
                        }
                        break;
                    case env.SVG_LAYER_TYPE.AP:
                        if(current_mode != "image") {
                            ShowApInfo(getId(), layerType, false);
                        }
                        break;
                    case env.SVG_LAYER_TYPE.OUT_WALL:
                        stroke_width = "0";
                        fill = "#000000"
                        break;
                }
        ...
        var dblClick = function(evt) {
            ...
            if(tagName === 'text' && current_mode !== 'textedit') {
                var pt = transformPoint( evt.clientX, evt.clientY, root_sctm );
...
//	$(window).mouseup(mouseUp);
        ...
        $(container).bind("mousewheel DOMMouseScroll", function(e){
            ...
            root_sctm = svgcontent.getScreenCTM().inverse();
            var pt = transformPoint( e.clientX, e.clientY, root_sctm );
            var bbox = {
                'x': pt.x,
                'y': pt.y,
                'width': 0,
...
// Functions relating to editing text elements
    var textActions = canvas.textActions = function() {
        ...
        function selectWord(evt) {
            ...
            var ept = transformPoint( evt.clientX, evt.clientY, root_sctm ),
...
// 					if(!index) return;
            ...
            toSelectMode: function(elem) {
                ...
                if(svgedit.browser.supportsNonScalingStroke()) {
                    var elem1 = svgedit.path.path.elem;
                    if (elem1) {
                        //elem1.removeAttribute('style');
                        elem1.style.vectorEffect = "";
                        svgedit.utilities.walkTree(elem1, function(elem2) {
                            //elem2.removeAttribute('style');
                            elem2.style.vectorEffect = "";
                        });
                    }
                }
                current_path = false;
...
// This function returns false if the set was unsuccessful, true otherwise.
    this.setSvgString = function(xmlString,nohistory) {
        try {
...
// Parameters:
// name - The given name
    this.createLayer = function(name,type,noHistory) {
        var new_layer = getCurrentDrawing().createLayer(name,type);
        if(!!!noHistory) {
...
// Parameters:
// name - The given name
    this.cloneLayer = function(name,type) {
        var batchCmd = new BatchCommand("Duplicate Layer");
        var new_layer = svgdoc.createElementNS(svgns, "g");
        var layer_title = svgdoc.createElementNS(svgns, "title");
        var layer_type = svgdoc.createElementNS(svgns, "desc");
        layer_title.textContent = name;
        layer_type.textContent = type;
        new_layer.appendChild(layer_title);
        new_layer.appendChild(layer_type);
        var current_layer = getCurrentDrawing().getCurrentLayer();
...
// true if the current layer was switched, otherwise false
    this.setCurrentLayer = function(name) {
        ...
        if (result) {
            this.setLayerShape(name);
            clearSelection();
            if((this.getMode() == "pathedit") || (this.getMode() == "textedit")) {
                this.setMode("select");
            }
        }
        return result;
	 ...
	 * AP图层为特定图形
	 */
    this.setLayerShape = function(layerName,layerType) {
        if(!!!layerType) {
            if(!!!layerName){
                layerName = getCurrentDrawing().getCurrentLayerName();
            }
            layerType = this.getLayerTypeByName(layerName);
        }
        var tools = [
            "tool_select",
            "tool_fhpath",
        ...
        switch(layerType) {
            case env.SVG_LAYER_TYPE.AP: {
                ...
                $('#tools_shapelib_show').removeClass('disabled');
                $('#shape_buttons').css('display', 'none');
                $('#tool_select').removeClass('disabled');
                $('#tool_zoom').removeClass('disabled');
                $('#tools_shapelib_show').attr('title',"AP图形");
                $('#tools_shapelib_show div').css('display', 'none');
                var tool_select = $("#tool_select");
                var is_tool_select_current = tool_select.hasClass("tool_button_current");
                $("#shapelib_AP").mouseup();
                if(is_tool_select_current){
                    tool_select.click();
                }
            }
                break;
            case env.SVG_LAYER_TYPE.OUT_WALL: {
                $('#tools_left li').addClass('disabled');
                $('#tools_shapelib_show').addClass('disabled');
                //$('#shape_buttons').css('display', 'none');
                $('#tool_select').removeClass('disabled');
                $('#tool_zoom').removeClass('disabled');
                $('#tools_rect_show').removeClass('disabled');
                //$('#tools_shapelib_show').attr('title',"AP图形");
                //$('#tools_shapelib_show div').css('display', 'none');
                var tool_select = $("#tool_select");
                var is_tool_select_current = tool_select.hasClass("tool_button_current");
                var tools_rect_show = $("#tools_rect_show");
                var is_tools_rect_show_current = tools_rect_show.hasClass("tool_button_current");
                //$("#shapelib_AP").mouseup();
                if((!is_tool_select_current) && (!is_tools_rect_show_current)){
                    tools_rect_show.mouseup();//svg-editor.js 1325
                }
                //if(is_tools_rect_show_current){
                //	tools_rect_show.mouseup();//svg-editor.js 1325
                //}
            }
                break;
            default: {
                $('#tools_left li').removeClass('disabled');
                $('#tools_shapelib_show').removeClass('disabled');
                $('#tools_shapelib_show').attr('title',"室内地图图形库");
                $('#shape_buttons').css('display', 'block');
                $('#tools_shapelib_show div').css('display', 'block');
            }
                break;
        }
    }
	 ...
	 * 选中选择工具
	 */
//this.tool_selectClick = function() {
//	var afterLayerName=$('.layersel .layername').text();
//	var afterLayerType=svgCanvas.getLayerTypeByName(afterLayerName);
//	if(afterLayerType=="AP"){
//		$("#tool_select").click();
//	}
//}
...
// true if the rename succeeded, false otherwise.
    this.renameCurrentLayer = function(newname) {
        ...
        if (drawing.current_layer) {
            ...
            if (!canvas.setCurrentLayer(newname)) {
                ...
                var i = this.getLayerNum(oldLayer);
    ...
    this.renameLayerByName = function(layername, newName) {
        var drawing = getCurrentDrawing();
        var layer = drawing.getLayerByName(layername);
        if (layer) {
            var layernum = this.getLayerNum(layer);
            var oldname = drawing.getLayerName(layernum);
            var len = layer.childNodes.length;
            for ( var i = 0; i < len; ++i) {
                var child = layer.childNodes.item(i);
                if (child && child.tagName == "title") {
                    while (!!child.firstChild) {
                        // child.firstChild
                        child.removeChild(child.firstChild);
                    }
                    drawing.all_layers[layernum][0] = svgedit.utilities
                        .toXml(newName);
                    child.textContent = newName;
                    var batchCmd = new BatchCommand("Rename Layer");
                    batchCmd.addSubCommand(new ChangeElementCommand(child, {
                        "#text" : oldname
                    }));
                    addCommandToHistory(batchCmd);
                    call("changed", [ layer ]);
    ...
    this.retypeLayerByName = function(layername, newType) {
        var typeTag = "desc";
        var drawing = getCurrentDrawing();
        var layer = drawing.getLayerByName(layername);
        if (layer) {
            var child = this.getLayerTypeElement(layer);
            if (child == null) {
                child = drawing.createElement(typeTag);
                layer.appendChild(child);
            }
            if (child != null) {
                oldType = child.textContent;
                while (!!child.firstChild) {
                    // child.firstChild
                    child.removeChild(child.firstChild);
                }
                child.textContent = newType;
                var batchCmd = new BatchCommand("Retype Layer");
                batchCmd.addSubCommand(new ChangeElementCommand(child, {
                    "#text" : oldType
                }));
                addCommandToHistory(batchCmd);
    ...
    this.renametypeLayerByName = function(layername, newName, newType) {
        var typeTag = "desc";
        var drawing = getCurrentDrawing();
        var layer = drawing.getLayerByName(layername);
        if (layer) {
            var batchCmd = new BatchCommand("Rename and Retype Layer");
            var ChangeElementCommandName = null;
            var ChangeElementCommandType = null;
            var layernum = this.getLayerNum(layer);
            var oldname = drawing.getLayerName(layernum);
            var len = layer.childNodes.length;
            for ( var i = 0; i < len; ++i) {
                ...
                if (child && child.tagName == "title") {
                    ...
                    drawing.all_layers[layernum][0] = svgedit.utilities
                        .toXml(newName);
                    child.textContent = newName;
                    ChangeElementCommandName = new ChangeElementCommand(child, {
                        "#text" : oldname
                    });
                }
            }
            ...
            if (child != null) {
                ...
                child.textContent = newType;
                ChangeElementCommandType = new ChangeElementCommand(child, {
                    "#text" : oldType
                });
            }
            if (ChangeElementCommandName != null
                || ChangeElementCommandType != null) {
                if (ChangeElementCommandName != null) {
                    batchCmd.addSubCommand(ChangeElementCommandName);
                }
                if (ChangeElementCommandType != null) {
                    batchCmd.addSubCommand(ChangeElementCommandType);
                }
                addCommandToHistory(batchCmd);
                call("changed", [ layer ]);
                return (true);
    ...
    this.getLayerTypeElement = function(layer) {
        var typeTag = "desc";
        if (layer) {
            var child = null;
            var len = layer.childNodes.length;
            var i = 0;
            while (i < len) {
                child = layer.childNodes.item(i);
                if (child && child.tagName == typeTag) {
                    return (child);
                }
                ++i;
            }
    ...
    this.getLayerTypeElementByName = function(layername) {
        var drawing = getCurrentDrawing();
        var layer = drawing.getLayerByName(layername);
        return (this.getLayerTypeElement(layer));
    }
    this.getLayerTypeByName = function(layername) {
        var typeElement = this.getLayerTypeElementByName(layername);
        return ((typeElement == null) ? "" : typeElement.textContent);
    }
    this.getLayerTypeByNum = function(n) {
        var drawing = getCurrentDrawing();
        var layername = drawing.getLayerName(n);
        return (this.getLayerTypeByName(layername));
    }
    this.getLayerNum = function(layer) {
        var drawing = getCurrentDrawing();
        var num = drawing.getNumLayers();
        var i = 0;
        while (i < num) {
            if (drawing.all_layers[i][1] == layer) {
                return (i);
            }
            ++i;
        }
    ...
    this.getLayerTypeList = function(){
        var layerTypeList = {};
        var layerName = null;
        var layerType = null;
        var drawing = getCurrentDrawing();
        ...
        var i = 0;
        while (i < num) {
            layerName = drawing.getLayerName(i);
            layerType = this.getLayerTypeByName(layerName);
            layerTypeList[layerType] = layerName;
            ++i;
        }
...
// Clears the current document.  This is not an undoable action.
    this.clear = function(noDefLayer) {
        pathActions.clear();
        ...
        canvas.current_drawing_ = new svgedit.draw.Drawing(svgcontent,idprefix);
        if(!!!noDefLayer) {
            var defaultLayerType = "OUT_W";
            for(var type in env.typeJson){
                defaultLayerType = type;
                break;
            }
            // create empty first layer
            canvas.createLayer("图层 1",defaultLayerType,true);
        }
