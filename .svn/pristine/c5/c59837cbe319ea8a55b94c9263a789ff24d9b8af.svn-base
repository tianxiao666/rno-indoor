var env = env || {};
/*env.mediaServer = "http://192.168.6.41:80/indooradmin/";
env.svgeditdir = "http://192.168.6.41:80/indooradmin/" +
    "js/svg-edit-2.6/";*/
env.include = true;
env.BUILDING_ID = "1";
env.FLOOR_WIDTH = parseFloat("133.17");
if ((!!!env.FLOOR_WIDTH) || isNaN(env.FLOOR_WIDTH)) {
    env.FLOOR_WIDTH = 96;
}
env.FLOOR_HEIGHT = parseFloat("155.14");
if ((!!!env.FLOOR_HEIGHT) || isNaN(env.FLOOR_HEIGHT)) {
    env.FLOOR_HEIGHT = 72;
}
env.typeJson = eval('(' +
    '{"OUT_W":"\u5916\u5899","TOILE":"\u536b\u751f\u95f4","ELEVA":"\u7535\u68af","STAIR":"\u697c\u68af","ROUTE":"\u8def\u5f84","BUSSI":"\u5546\u5bb6","AP":"AP","POI":"POI"}'
    + ')');
env.DEFAULT_PLANEGRAPH = eval('(' +
    '{"DW_SCALE":0.15,"DW_UNIT":"px","BACKGROUD_COLOR":"#FFFFFF","STATUS":"E"}'
    + ')');
env.ViewBoxWidth = Math.round(env.FLOOR_WIDTH /
    env.DEFAULT_PLANEGRAPH.DW_SCALE);
env.ViewBoxHeight = Math.round(env.FLOOR_HEIGHT /
    env.DEFAULT_PLANEGRAPH.DW_SCALE);
env.svgeditoffset = 0;
env.SVG_LAYER_TYPE = eval('(' +
    '{"OUT_WALL":"OUT_W","AP":"AP","POI":"POI"}' + ')');
env.isFullScreen = false;
(function(){
    console.log("第一个初始化(function(){...");
    var POIFORMLISTJSONSTR = '';
    if (POIFORMLISTJSONSTR != "") {
        env.PoiFormListJson = eval('(' + POIFORMLISTJSONSTR + ')');
    }
    else {
        env.PoiFormListJson = {};
    }
    var APFORMLISTJSONSTR = '';
    if (APFORMLISTJSONSTR != "") {
        env.ApFormListJson = eval('(' + APFORMLISTJSONSTR + ')');
    }
    else {
        env.ApFormListJson = {};
    }
    var SVGPICLISTJSONSTR = '';
    if (SVGPICLISTJSONSTR != "") {
        env.SvgPicListJson = eval('(' + SVGPICLISTJSONSTR + ')');
    }
    else {
        env.SvgPicListJson = {};
    }
...
$(document).ready(function(){
    console.log("第二个初始化$(document).ready(function(){...");
    /**
     * 静态页面传递参数分解
     */
    ...
    rePositionSvgEdit();
    if (!!env.include) {
        console.log("进入env.include="+env.include);
        init_embed();
        init_fullscreen();
    }
});
...
function loadSvg(DRAW_MAP_ID){
    ...
    var BUILDING_ID = $("#BUILDING_ID").val();
    var FLOOR_ID = $("#FLOOR_ID").val();
    console.log("进入loadSvg==="+DRAW_MAP_ID);
            ...
            callback: function(PNGBASE64){
                ...
                if (paramsJson != null) {
                    ...
                    $("#SELECTIMAGEFORM").ajaxSubmit({
                        ...
                        type: "POST",
                        url:
                            "ea.php?r=SvgMgr/AjaxUploadSvgAndSavePng",
                        dataType: 'json',
                        // data: paramsJson,
                        success: function(json){
                            if (json.DRAW_MAP_ID) {
