	...
	a.flexslider=function(c,b){
		var d=c;d.init=function(){
			...
			if(d.vars.controlNav){
				if(d.manualExists){d.controlNav=d.manualControls}
					...
					var g=a('<ol class="login_slideNav"></ol>');
					var k=1;for(var l=0;l<d.count;l++){g.append("<li><a>"+k+"</a></li>");k++}
					if(d.containerExists){a(d.controlsContainer).append(g);
						d.controlNav=a(".login_slideNav li a",d.controlsContainer)
					}else{
						d.append(g);
						d.controlNav=a(".login_slideNav li a",d)
					}
				}
			...
			if(d.vars.keyboardNav&&a("ul.slides").length==1){
				a(document).keyup(function(i){
					if(d.animating){return}
						...
						if(i.keyCode!=39&&i.keyCode!=37){return}
						else{
							if(i.keyCode==39){var j=d.getTarget("next")}
							else{if(i.keyCode==37){var j=d.getTarget("prev")}}
							if(d.canAdvance(j)){d.flexAnimate(j,d.vars.pauseOnAction)}
						}
					}
				})
			}
			if(d.vars.slideshow){
				if(d.vars.pauseOnHover&&d.vars.slideshow){
					d.hover(function(){d.pause()},function(){d.resume()})
				}
				...
				d.each(function(){
					var i,j=20;isMoving=false;function o(){
						...
						if(isMoving){
							...
							if(Math.abs(q)>=j){
								...
								if(d.canAdvance(r)){d.flexAnimate(r,d.vars.pauseOnAction)}
							}
						}
					}function n(p){
		...
		d.flexAnimate=function(f,e){
			if(!d.animating){
				...
				if(!d.vars.animationLoop){
					if(f==0){d.directionNav.removeClass("disabled").filter(".prev").addClass("disabled")
					}else{
						if(f==d.count-1){d.directionNav.removeClass("disabled").filter(".next").addClass("disabled");
							d.pause();d.vars.end(d)
						}else{
				...
				if(d.vars.animation.toLowerCase()=="slide"){
					...
					d.slides.eq(f).fadeIn(d.vars.animationDuration,function(){
						d.animating=false;
						d.currentSlide=f;d.vars.after(d)})
				}
			}
		};
		...
		d.canAdvance=function(e){
			if(!d.vars.animationLoop&&d.atEnd){
				if(d.currentSlide==0&&e==d.count-1&&d.direction!="next"){return false}
				else{
					if(d.currentSlide==d.count-1&&e==0&&d.direction=="next"){return false}
					else{return true}
				}
			}else{return true}
		};
	...
	a.fn.flexslider=function(b){
		return this.each(function(){
			if(a(this).find(".slides li").length==1){
				...
				if(a(this).data("flexslider")!=true){new a.flexslider(a(this),b)}
			}
		})
	}
})(jQuery);